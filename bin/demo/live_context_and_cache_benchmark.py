...
from omlxc.dataplane.adaptive_kv_quant import AdaptiveKVQuantizer
from omlxc.dataplane.context_compressor import ContextOptimizer
from omlxc.dataplane.hierarchical_cache import HierarchicalCacheCoordinator
...
def main() -> None:
    ...
    coordinator.record_and_cache_turn(
        prompt_text=system_prompt + "\n用户：请分析当前架构健康度。",
        token_seq=tokens_turn_1,
        response_text="架构健康度 100%，所有测试全绿，三节点负载均衡正常。",
    ...
    speedup = plan_1.estimated_ttft_ms / max(0.1, plan_2.estimated_ttft_ms)
    print(f"  - 预估 TTFT 首字延迟: {plan_2.estimated_ttft_ms} ms (提速 {speedup:.1f}x)")
    ...
    saved_pct = (1.0 - quant_plan.compression_ratio) * 100
    print(f"  - 压缩比率: {quant_plan.compression_ratio * 100:.1f}% (节省 {saved_pct:.1f}% 显存)")

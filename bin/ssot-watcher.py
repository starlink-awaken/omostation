...
class SSOTWatcher:
    ...
    def __init__(self):
        self.ssot_files = [SSOTFile(name, path) for name, path in SSOT_FILES]
    ...
    def log_changes(self, changes: list[tuple[str, str, str]], author: str, reason: str):
        """记录变更到审计日志"""
        current_state = self.capture_current_state()
        last_state = self.load_last_state()
    ...
    def get_audit_log(self, limit: int | None = None) -> list[AuditLogEntry]:
        """获取审计日志"""
        entries = []

...
def get_project_layer(project_name: str, contract: dict) -> str | None:
    """获取项目所属层"""
...
def get_project_from_import(import_name: str, project_dirs: set[str]) -> str | None:
    """从导入名提取可能的项目名.
...
class DependencyViolation:
    ...
    def __str__(self) -> str:
        return f"{self.from_project} ({self.from_layer}) → {self.to_project} ({self.to_layer}) in {self.file_path}"
...
def main():
    ...
    parser.add_argument("--project", type=Path, help="Check only a single project")

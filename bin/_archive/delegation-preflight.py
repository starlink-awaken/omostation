...
is only visible post-hoc. This tool checks the four critical surfaces up front:
  ...
  4. omlxc_endpoint_reachable  — HTTP GET {baseURL}/models answers
...
def _http_get(base_url: str, path: str, timeout: int = HTTP_TIMEOUT) -> tuple[int | None, bytes, Exception | None]:
...
def _extract_model_ids(body: bytes) -> list[str]:
    """从 OpenAI 兼容 /v1/models 响应提取模型 id 列表."""
    try:
...
def check_omlxc_endpoint_reachable(ctx: SimpleNamespace) -> tuple[str, str]:
    ...
    status, _body, err = _http_get(base_url, "/models")
    if status is not None:
        return "PASS", f"endpoint 可达 (HTTP {status}): {base_url}/models"
    return "FAIL", f"endpoint 不可达: {base_url}/models — {err}"
...
def check_gateway_models_available(ctx: SimpleNamespace) -> tuple[str, str]:
    ...
    status, body, err = _http_get(base_url, "/models")
    if status is None:
        return "WARN", f"网关 /v1/models 不可达 (信息性): {err}"
    listed = {normalize(m) for m in _extract_model_ids(body)}
...
def main() -> int:
    ...
    critical_failed = any(r["status"] == "FAIL" and r["critical"] for r in results)
    exit_code = 1 if critical_failed else 0

    if args.json:
        payload = {
            "preflight": "FAIL" if critical_failed else "PASS",
            "exit_code": exit_code,
            "base_url": base_url,

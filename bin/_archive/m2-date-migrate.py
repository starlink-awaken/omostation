...
用法:
    uv run --with pyyaml python bin/_archive/m2-date-migrate.py
    uv run --with pyyaml python bin/_archive/m2-date-migrate.py --apply
"""
...
def main() -> int:
    ...
    if not args.apply:
        print("\n推荐:")
        print("  uv run --with pyyaml python bin/_archive/m2-date-migrate.py --apply")
        print("  uv run --with pyyaml python tests/integration/m4_metamodel/run_all.py")
    else:
        print("\n验证:")
        print("  uv run --with pyyaml python bin/mof/mof-bootstrap.py check_5")
    return 0

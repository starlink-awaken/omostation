...
def check_permission(
    *, requester: str, resource: str, action: str, purpose: str, context: str = "normal"
) -> dict[str, Any]:
    ...
    layers.append(
        {"layer": "L1_identity", "result": "pass" if identity_ok else "deny", "detail": f"requester={requester}"}
    )
    if not identity_ok:
    ...
    layers.append({"layer": "L2_relationship", "result": "pass", "detail": f"baseline={baseline}"})
    ...
    if "confidential" == sensitivity and "full" not in rel_sensitivity:
        layers.append(
            {"layer": "L3_sensitivity", "result": "deny", "detail": f"resource={sensitivity}, access={rel_sensitivity}"}
        )
        return _final_decision(layers, "deny")
    elif "personal" == sensitivity and "degraded" in rel_sensitivity:
        layers.append({"layer": "L3_sensitivity", "result": "degrade", "detail": "personal → degraded view"})
    else:
    ...
    layers.append({"layer": "L4_purpose", "result": "pass" if purpose_ok else "deny", "detail": f"purpose={purpose}"})
    if not purpose_ok:
        return _final_decision(layers, "deny")

    # L5: Temporal
    if context == "emergency" and requester in ("self", "spouse"):
        layers.append({"layer": "L5_temporal", "result": "escalate", "detail": "emergency context — elevated access"})
    else:
    ...
    if action == "write" and "write" not in rel_action:
        layers.append({"layer": "L6_action", "result": "deny", "detail": f"write not allowed for {requester}"})
        return _final_decision(layers, "deny")
    elif action == "read" and "read" not in rel_action:
        layers.append({"layer": "L6_action", "result": "deny", "detail": f"read not allowed for {requester}"})
        return _final_decision(layers, "deny")
    layers.append({"layer": "L6_action", "result": "pass", "detail": f"action={action} allowed"})

    # L7: Delegation
    layers.append({"layer": "L7_delegation", "result": "pass", "detail": f"delegation={baseline['delegation']}"})
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = check_permission(
        requester=args.requester, resource=args.resource, action=args.action, purpose=args.purpose, context=args.context
    )

...
def assess(items: list[dict], root: Path, now: datetime) -> dict:
    ...
    for item in items:
        ...
        if state in TERMINAL_STATES:
            if not has_verifiable_evidence(item, root):
                needs_reassessment.append(
                    {"id": iid, "file": item.get("_file"), "state": state,
        ...
        if last is None:
            overdue.append({"id": iid, "file": item.get("_file"), "state": state,
                            "days_since_review": None, "interval_days": interval,
                            "reason": "从未复审"})
            continue
        days = (now - last).days
        if days > interval:
            overdue.append({"id": iid, "file": item.get("_file"), "state": state,
                            "days_since_review": days, "interval_days": interval,
...
def main() -> int:
    ...
    report = assess(items, root, datetime.now(timezone.utc))

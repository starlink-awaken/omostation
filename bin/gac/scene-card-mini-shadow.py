#!/usr/bin/env python3
"""scene-card-mini-shadow: 检测 shadow 场景卡并提议 mini-trial 升级路径.

BET-Y1Q3-T10-08 / 项目战略 v1 §1 落地:
  9 张 shadow 场景卡 (0 samples) 全部卡在 30-samples 校准门.
  现实是: 主人一周走不了一次 30-sample 循环.

mini-shadow 模式:
  - 3 个主人手勾"有用" → mini-trial 通过
  - 写入 `lifecycle_gate.mini_trial` 块
  - 升级到 assisted (而不是原 assisted 的 30 samples)

用法:
  python3 bin/gac/scene-card-mini-shadow.py --list       # 列出所有 shadow 卡
  python3 bin/gac/scene-card-mini-shadow.py --record <scene_id> --outcome useful  # 记录主人勾选
  python3 bin/gac/scene-card-mini-shadow.py --eligible    # 列出可升级的卡 (≥3 mini-samples)
"""
...
MINI_THRESHOLD = 3  # mini-trial 通过所需主人勾选次数
...
def _load_yaml_simple(path: Path) -> dict[str, Any]:
    """Read scene card YAML without PyYAML dependency (basic key:value).

    For richer fields (lists, nested), graceful fallback — full YAML
    editing is not required for the mini-trial flow. We just need to
    read/update `lifecycle_gate.mini_trial` and `lifecycle`.
    """
    try:
        import yaml
...
def _read_mini_trial(body: dict[str, Any]) -> dict[str, Any]:
    """Read the mini_trial block from a scene card body (or empty)."""
...
def _shadow_age_days(body: dict[str, Any]) -> int:
    """Days since shadow_start (set when card entered shadow)."""
    gate = body.get("lifecycle_gate") or {}
    if not isinstance(gate, dict):
        return 0
...
def list_shadow_cards() -> list[dict[str, Any]]:
    """Return all shadow scene cards with mini-trial status."""
...
def record_mini_sample(scene_id: str, outcome: str) -> dict[str, Any]:
    """Append a mini-trial sample to a shadow scene card (file edit)."""
...
def _write_yaml_simple(path: Path, body: dict[str, Any]) -> None:
    """Update only the lifecycle_gate.mini_trial block in-place (textual).

    Preserves the existing lifecycle_gate structure (target, blocking_bet,
    thresholds, measured_*, status, rationale) — only adds/updates
    `mini_trial` key. We deliberately avoid full YAML dump to keep edits
    minimal and preserve the human-authored narrative above/below.
    """
    ...
    if "lifecycle_gate:" not in text:
        # No lifecycle_gate block — append a fresh one.
        if "lifecycle: shadow" in text:
            text = text.rstrip() + "\n\nlifecycle_gate:\n" + (new_mini_text or "  mini_trial: {}\n")
        path.write_text(text, encoding="utf-8")
        return

    # Find the lifecycle_gate block bounds (top-level key).
    lines = text.splitlines()
    start = end = None
    for i, line in enumerate(lines):
        if line.startswith("lifecycle_gate:"):
            start = i
            # Walk forward; block ends at next 0-indent non-comment line.
    ...
    # Inside the lifecycle_gate block, look for `mini_trial:` subsection.
    ...
    if new_mini_text:
        # Always emit the mini_trial: header, then the body
...
def _format_mini_trial_inner(mini: dict[str, Any]) -> str:
    """Render mini_trial as YAML text (4-space indent, since nested under lifecycle_gate)."""
    if not mini:
        return ""
    samples = mini.get("samples") or []
    lines: list[str] = []
...
def _format_mini_trial_block(mini: dict[str, Any]) -> str:
    """Legacy: full 2-space block for top-level append (rare path)."""
    if not mini:
        return ""
    return "  mini_trial:\n" + _format_mini_trial_inner(mini)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="scene-card mini-shadow: 3-sample 升级门替代 30-sample 校准门"
    )
    parser.add_argument("--list", action="store_true", help="列出所有 shadow 场景卡")
    parser.add_argument(
        "--eligible",
        action="store_true",
        help="列出已达成 mini-trial 门槛 (≥3 useful) 的卡",
    )
    parser.add_argument("--record", help="记录主人对某张卡的勾选 (--outcome 必填)")
    parser.add_argument(
        "--outcome",
        choices=["useful", "not_useful"],
        help="主人评估: useful 或 not_useful",
    )

#!/usr/bin/env python3
"""Install and verify the governed Documents MCP entry in ZCode."""
...
class ConfigError(RuntimeError):
    """A stable, user-facing ZCode configuration failure."""
...
def _documents_root(domain_registry: Path, instruction_file: str) -> Path:
    ...
    try:
        metadata = gateway.lstat()
    except OSError as exc:
        raise ConfigError(f"Documents gateway unavailable: {gateway}") from exc
    if not stat.S_ISREG(metadata.st_mode):
        raise ConfigError(f"Documents gateway must be a regular file: {gateway}")
    return root
...
def _load_contract(project_registry: Path, domain_registry: Path) -> ConfigContract:
    ...
    if not isinstance(clients, dict) or not isinstance(clients.get("zcode"), dict):
        raise ConfigError("clients.zcode must be a mapping")
    client = clients["zcode"]
    if client.get("instruction_file") != "AGENTS.md":
        raise ConfigError("clients.zcode.instruction_file must be AGENTS.md")
    if client.get("mcp_scope") != "user":
        raise ConfigError("clients.zcode.mcp_scope must be user")

    contract = client.get("config_contract")
    if not isinstance(contract, dict):
        raise ConfigError("clients.zcode.config_contract must be a mapping")
    ...
    documents = _documents_root(domain_registry, "AGENTS.md")
...
def _check_settings(
    ...
    try:
        ...
        server, projection = next(iter(expected["mcp"]["servers"].items()))
        if servers.get(server) != projection:
            errors.append(f"mcp.servers.{server} does not match the Workspace contract")

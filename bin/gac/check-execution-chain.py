#!/usr/bin/env python3
"""Execution-chain coverage — fuse script-registry, ci-surfaces, and cron jobs.

An item is in the chain if it appears in at least one of those three existing
inventories. Legal triggers (hook / CI / cron / manual) are derived from the
same records. Live items with no classified trigger are warnings. Active items
injected via extra_active that appear in none of the inventories fail-closed.
...
def check(
    *,
    ...
    """Fuse the three inventories and classify triggers.

    extra_active: claimed-active paths that must appear in at least one
    ...
    serial_items = viewed
    if len(serial_items) > 40:
        serial_items = viewed[:40]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            "  inventories "
            f"script_registry={result['inventories']['script_registry']} "
            f"ci_surfaces={result['inventories']['ci_surfaces']} "
            f"cron_jobs={result['inventories']['cron_jobs']} "

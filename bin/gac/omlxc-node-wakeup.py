#!/usr/bin/env python3
# bin/gac/omlxc-node-wakeup.py — AetherForge 从机物理节点网络唤醒 (WoL) 自愈代理
#
# 从 project-registry.yaml 读取 compute_nodes MAC 地址与 LAN IP，并发送 Magic Packet 唤醒包。
#
# 用法:
#   python3 bin/gac/omlxc-node-wakeup.py --node mac-mini-M4
#   python3 bin/gac/omlxc-node-wakeup.py --all
...
def main() -> int:
    ...
    if args.node:
        if args.node not in nodes:
            print(
                f"❌ 节点 '{args.node}' 未在注册表中定义。可用节点: {list(nodes.keys())}",
                file=sys.stderr,
            )
            return 1

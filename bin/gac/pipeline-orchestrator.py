...
def scan_calendar(calendar_path: Path) -> list[dict]:
    ...
    if not calendar_path.exists():
        return []
    cmd = [sys.executable, str(REPO / "bin" / "bc-os" / "signal_router.py"), "--calendar", str(calendar_path), "--json"]
    result = run_cmd(cmd)
...
def trigger_journey(scene_id: str, signal: dict) -> dict:
    ...
    result = run_cmd(
        [
            sys.executable,
            str(REPO / "bin" / "gac" / "signal-scene-connector.py"),
            "--scene",
            scene_id,
        ]
    )
...
def record_value(task_id: str, minutes: int, description: str) -> dict:
    ...
    result = run_cmd(
        [
            sys.executable,
            str(REPO / "bin" / "gac" / "value-tracker.py"),
            "--record",
            str(minutes),
            "--task",
            task_id,
...
def run_evolution_cycle() -> dict:
    ...
    result = run_cmd(
        [
            sys.executable,
            str(REPO / "bin" / "gac" / "auto-evolution-engine.py"),
            "--cycle",
        ]
    )

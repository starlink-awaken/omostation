...
DEFAULT_SETTINGS = Path.home() / "Library" / "Application Support" / "Claude" / "claude_desktop_config.json"
...
def _load_contract(project_registry: Path, domain_registry: Path) -> ConfigContract:
    ...
    for field, value in expected.items():
        if type(contract.get(field)) is not type(value) or contract.get(field) != value:
            ...
            raise ConfigError(f"clients.claude.config_contract.{field} must be {rendered}")
    ...
    if contract.get("managed_mcp_server") != server:
        raise ConfigError("clients.claude.config_contract.managed_mcp_server must match workspace_mcp.server")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "install" and args.settings_path == DEFAULT_SETTINGS and _claude_desktop_running():
            print(
                json.dumps(
                    _envelope(
                        False,

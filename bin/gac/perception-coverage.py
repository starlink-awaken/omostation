...
Outputs current Harness 7 probes + 25 routes + 17 data sources coverage.
...
def generate_json() -> dict:
    ...
    route_score = 2  # 25/25 routes
    overall = round(source_score + probe_score + route_score, 1)
    ...
    return {
        ...
        "data_sources": {
            ...
            "missing": total_sources - healthy_sources - partial_sources,
        },
...
def generate_report() -> str:
    ...
    lines = [
        ...
        "## Probe Coverage",
        "",
        "| Probe | Topic | Healthy | Partial | Total | Coverage |",
        "|-------|-------|---------|---------|-------|----------|",
    ]
    for p in data["probes"]["items"]:
        lines.append(f"| {p['id']} | {p['topic']} | {p['healthy']} | {p['partial']} | {p['total']} | {p['coverage']*100:.0f}% |")
    ...
    lines += [
        "", "## Summary",
        f"- Data Sources: {data['data_sources']['healthy']} healthy, {data['data_sources']['partial']} partial, {data['data_sources']['total']} total",
...
def main():
    ...
    if args.json:
        ...
        data = generate_json()
        print(f"Score: {data['overall_score']}/10 | Probes: {data['probes']['healthy']}/{data['probes']['total']} | Sources: {data['data_sources']['healthy']}h/{data['data_sources']['partial']}p/{data['data_sources']['total']}")

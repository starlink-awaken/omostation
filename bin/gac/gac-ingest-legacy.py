...
用法:
  python3 bin/gac/gac-ingest-legacy.py              # dry-run 打印 indexed 条目
  python3 bin/gac/gac-ingest-legacy.py --write      # append indexed 到 governance-checks.yaml
  python3 bin/gac/gac-ingest-legacy.py --json       # JSON 输出 (机器消费)
...
def main() -> int:
    ...
    if args.update_relates:
        ...
        if n:
            print("   验证: python3 bin/gac/gac-validate.py --gate")
        return 0
    ...
    if args.check:
        ...
        if drift["ok"]:
            ...
            print("  → 修复: python3 bin/gac/gac-ingest-legacy.py --write")
        return 0 if drift["ok"] else 1

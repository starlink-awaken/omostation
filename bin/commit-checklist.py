...
def evaluate(staged: list[str], strict: bool = True) -> tuple[list[CheckResult], list[str]]:
    ...
    for item in ITEMS:
        ...
        if item.id == "docs-capability-sync":
            ...
            sub_files = [s for s in staged if "/.gitmodules" in s or s.startswith("projects/")]
            if sub_files:
            ...
            if docs_files:
                # Check ADR index updated if new decision added
                idx = ROOT / ".omo/_knowledge/decisions/INDEX.md"
                if idx.exists():
                    ...
                    for f in docs_files:
                        ...
                        if p.exists() and p.stat().st_mtime > idx_mtime:
                            detail = "New ADR/decision may need INDEX update"
                            missing_flag = True
                            break
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    staged = args.files if args.files else _staged_files()

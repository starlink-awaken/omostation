...
if configured_dist:
    ...
    COCKPIT_UI_DIR = (
        configured_path.parent if configured_path.name == "dist" else configured_path
    )
else:
...
def try_build() -> bool:
    print(
        "ℹ cockpit-ui static assets (dist/index.html) missing. Attempting self-heal build..."
    )

    # 尝试使用 bun 或是 npm 进行构建
    managers = [("bun", ["bun", "run", "build"]), ("npm", ["npm", "run", "build"])]
    ...
    for name, cmd in managers:
        try:
            ...
            # 运行 build 进程
            res = subprocess.run(
                cmd, cwd=COCKPIT_UI_DIR, capture_output=True, text=True, check=False
            )
            if res.returncode == 0:
...
def main() -> int:
    if not COCKPIT_UI_DIR.is_dir():
        print(
            f"⏭️  Skip: cockpit-ui directory not found at {COCKPIT_UI_DIR} (not a submodule, not in CI)"
        )
        return 0
    ...
    print(
        "👉 Human decision needed: Please navigate to projects/cockpit-ui and execute:"
    )

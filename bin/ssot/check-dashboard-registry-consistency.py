...
def main() -> int:
    ...
    dash_partial = sum((cat or {}).get("partial", 0) if isinstance(cat, dict) else 0 for cat in categories.values())

    # registry 端: lifecycle_state=partial 的 item 数
    items = reg.get("items") or []
    reg_partial = sum(1 for it in items if isinstance(it, dict) and it.get("lifecycle_state") == "partial")
    ...
    print("   治本: 看 ISC-50 (两套看板 SSOT 统一 ADR) — dashboard 停更或漂移导致 partial 计数分叉")
    return 1

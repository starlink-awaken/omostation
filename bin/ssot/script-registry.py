...
def validate() -> None:
    ...
    actual_scripts = set()
    # 只验证 git tracked 脚本 (2026-08-27 深度复盘 F7: iterdir 扫文件系统会把并行
    # agent 的进行中未提交脚本算进来, 冻结所有人的 push — 只问责已提交面)
    import subprocess
    ls = subprocess.run(
    ...
    for line in ls.stdout.splitlines():
        ...
        if len(parts) > 1 and (parts[1] == "_registry" or parts[1].startswith("_")):
            continue

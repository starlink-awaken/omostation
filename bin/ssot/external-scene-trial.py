...
def _trial_plan(
    plan: Mapping[str, Any], scene_card: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    metric_result = {
        ...
        "measurement_ref": _opaque(
            metric.get("measurement_ref"), "metric.measurement_ref"
        ),
    }
    ...
    return {
        ...
        "preflight_ref": _opaque(
            plan.get("preflight_ref")
            or f"ref://external-scene-trial/preflight/{_digest(plan)[:24]}",
            "preflight_ref",
        ),
...
def build_scene_trial(
    ...
    *,
    ...
    intake_module = _load_module(
        root / "bin/ssot/scene-card-intake.py", "scene_card_intake_for_trial"
    )
    preflight_module = _load_module(
        root / "bin/ssot/external-activation-preflight.py",
        "external_preflight_for_trial",
    )
    intake = intake_module.build_intake(dict(scene_card))
    preflight = preflight_module.build_preflight(
        dict(scene_card), dict(catalog), now=now
    )
    if intake["status"] == "blocked" or preflight["status"] == "blocked":
        return {
            ...
            "missing_fields": sorted(
                set(intake["missing_fields"] + preflight["missing_fields"])
            ),
            "preflight": preflight,
            "side_effects": {
    ...
    return {
        "schema": "external-scene-trial-result/v1",
        "status": "proposal_only",
        "trial": trial,
        "preflight": preflight,
    }
...
def _run_omo(root: Path, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    environment["PYTHONPATH"] = (
        f"{omo_src}{os.pathsep}{environment.get('PYTHONPATH', '')}".rstrip(os.pathsep)
    )
    ...
    if completed.returncode:
        raise SceneTrialInputError(
            completed.stderr.strip() or "OMO scene trial recorder failed"
        )
    try:
        result = json.loads(completed.stdout)
    except json.JSONDecodeError as exc:
        raise SceneTrialInputError(
            "OMO scene trial recorder returned invalid JSON"
        ) from exc
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--record", action="store_true", help="persist through the OMO broker"
    )

...
def check_file(
    ...
    if surface.get("metadata") == "frontmatter":
        ...
        valid_lifecycles_by_schema = surface.get("valid_lifecycles_by_schema") or {}
        valid_lifecycles = set(
            valid_lifecycles_by_schema.get(str(metadata.get("schema_version", "")))
            or surface.get("valid_lifecycles")
            or registry.get("metadata", {}).get("valid_lifecycles", [])
        )

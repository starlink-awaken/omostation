        ...
        for f in (proj_dir / "src").rglob("*.py"):
            ...
            for pat, pat_name in PORT_PATTERNS:
                for m in pat.finditer(text):
                    ...
                    is_env_fallback = (
                        "os.environ.get(" in context or "os.environ.get (" in context
                    )
...
def main() -> int:
    ...
    p.add_argument(
        "--threshold", type=int, default=0, help="unregistered port 阈值 (默认 0, hard)"
    )
    p.add_argument(
        "--env-var-check",
        action="store_true",
    ...
    for port in sorted(hardcoded.keys() & registered):
        ...
        env_fallback_usages.append(
            {"port": port, "count": len(env_fb), "env_var": env_vars.get(port)}
        )
        bare_hardcoded_usages.append(
            {"port": port, "count": len(bare), "env_var": env_vars.get(port)}
        )
        # 输出 env-only 端口 bare 硬编码 warning
        if args.env_var_check and bare and port in (7422, 7456, 8090):
            for e in bare[:3]:
                print(
                    f"  ⚠️  env-only port {port} bare hardcoded: {e['file']}:{e['line']} ({e['pattern']})"
                )
    ...
    if args.json:
        ...
        print(
            f"  env var fallback (P77-7-2 OK): {summary['env_fallback_usages_count']}"
        )
        print(
            f"  bare hardcoded (修真修真, should use env var): {summary['bare_hardcoded_usages_count']}"
        )
        print(f"  env vars defined in SSOT: {summary['env_vars_defined']}")
        print(
            f"  legacy usages (external/standard, 豁免): {summary['legacy_usages_count']}"
        )
        print()
        ...
        if summary["unregistered"] > args.threshold:
            ...
            print(
                f"✅ hardcoded ports {summary['unregistered']} ≤ threshold {args.threshold}"
            )

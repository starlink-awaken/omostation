...
用法:
  python3 bin/ssot/check-toolbox-ssot.py            # 检查所有 ToolBox docs
...
def get_ssot_values() -> dict:
    ...
    try:
        data = json.loads(
            (
                WORKSPACE / "projects/kairon/packages/forge/tools-registry.json"
            ).read_text()
        )
    ...
    try:
        if HAS_YAML:
            data = yaml.safe_load(
                (WORKSPACE / "docs/project-registry.yaml").read_text()
            )
        ...
        for pattern, label, reason in stale:
            for i, line in enumerate(content.splitlines(), 1):
                if re.search(pattern, line):
                    if any(
                        kw in line
                        for kw in ["归档", "archived", "historical", "历史", "→", "->"]
                    ):
                        continue
...
def main() -> int:
    ...
    ap.add_argument(
        "--no-fail", action="store_true", help="Don't fail on findings (advisory)"
    )
    args = ap.parse_args()
    ...
    if args.json:
        print(
            json.dumps(
                {"ssot": ssot, "findings": findings}, indent=2, ensure_ascii=False
            )
        )
    else:

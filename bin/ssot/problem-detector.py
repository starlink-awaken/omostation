...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--once", action="store_true", help="write last-run state (launchd/cron 调度用)"
    )
    ...
    if args.json:
        ...
        print(
            f"Problem Detection: {result['total_problems']} problems ({result['by_severity']})"
        )
        for p in result["problems"]:
            print(
                f"  [{p.get('severity', '?').upper():6s}] {p['type']}: {p.get('detail', '')}"
            )

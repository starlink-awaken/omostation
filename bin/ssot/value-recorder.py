...
这是达成 value 维度的唯一必要操作.

用法 (每次 workflow 结束后运行):
    python3 value-recorder.py --review 120 --saved 300 --verdict accept

参数:
    --review  你审核输出花了多少秒 (如 120 = 2分钟)
    --saved   系统帮你节省了多少秒 (如 300 = 5分钟)
    --verdict accept=接受 / edit=编辑后接受 / reject=拒绝

示例:
    # 花了 2 分钟审核, 节省了 5 分钟, 接受了输出
    python3 value-recorder.py --review 120 --saved 300 --verdict accept

    # 花了 30 秒审核, 节省了 2 分钟, 编辑后接受
    python3 value-recorder.py --review 30 --saved 120 --verdict edit
"""
...
def count_episodes() -> dict:
    ...
    return {
        ...
        "qualifying_rate": round(qualifying / total, 2) if total > 0 else 0,
    }


def main():
    import argparse
    parser = argparse.ArgumentParser(description="Value Recorder — 记录时间节省")
    parser.add_argument("--review", type=int, help="审核时间 (秒)")
    parser.add_argument("--saved", type=int, help="节省时间 (秒)")
    parser.add_argument("--verdict", choices=["accept", "edit", "reject"], help="裁决")
    ...
    if args.status:
        ...
        print()
        print("  目标: 4 周 × 每周 3 个 qualifying = 12 个")
        if stats['qualifying'] >= 12:
            print("  ✓ 目标已达成!")
        else:
            print(f"  还需: {12 - stats['qualifying']} 个 qualifying episodes")
        return

    if args.review is None or args.saved is None or args.verdict is None:
        print("请提供 --review, --saved, --verdict 参数")
        print("示例: python3 value-recorder.py --review 120 --saved 300 --verdict accept")
        sys.exit(1)
    ...
    stats = count_episodes()
    print(f"  累计: {stats['qualifying']}/{stats['total']} qualifying")
    print(f"  目标: 12 个 qualifying episodes (4周 × 3/周)")

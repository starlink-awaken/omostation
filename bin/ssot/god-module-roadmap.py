...
def build_roadmap(structure: dict, top_n: int) -> dict:
    ...
    if import_count > 30:
        steps.append(
            f"Step 1: 拆分 imports ({import_count} 个, 可分组: 标准库/第三方/项目内)"
        )
    if classes:
        biggest = max(classes, key=lambda c: c.get("lines", 0))
        steps.append(
            f"Step 2: 拆最大类 `{biggest['name']}` ({biggest.get('lines', 0)}L, "
...
def main() -> int:
    ...
    if roadmap["top_classes"]:
        ...
        for c in roadmap["top_classes"][: args.top]:
            ...
            print(
                f"   {c['name']:<40s} line={c['lineno']:>5d}  lines={lines:>4d}  methods={methods}"
            )
        print()
    ...
    if roadmap["top_functions"]:
        ...
        for f in roadmap["top_functions"][: args.top]:
            ...
            args_n = f.get("args", 0)
            print(
                f"   {f['name']:<40s} line={f['lineno']:>5d}  lines={lines:>4d}  args={args_n}"
            )
        print()
    ...
    if roadmap["candidates"]:
        ...
        for c in roadmap["candidates"]:
            print(
                f"   priority={c['priority']:>3d}  {c['name']:<40s} line={c['lineno']:>5d}"
            )
        print()

...
Usage:
  python3 bin/governance-convergence-lint.py [--rule <name>] [--json]
"""
...
def check_matrix_consistency() -> tuple[list[str], list[str]]:
    ...
    errors, warnings = [], []
    lint_script = WORKSPACE / "bin/matrix-consistency-lint.py"
    if not lint_script.exists():
        warnings.append("R-GOV-4/6 WARN: bin/matrix-consistency-lint.py not found")
        return errors, warnings
    try:
        r = subprocess.run(

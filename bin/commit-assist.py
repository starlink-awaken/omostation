...
def query_aetherforge(model: str, prompt: str, timeout: int) -> str | None:
    ...
    payload = json.dumps(
        {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "你是 commit message 生成器. 输出必须是 Conventional Commits: <type>(<scope>): <subject> + 3-5 行 body. Chinese OK. 不要 preamble / markdown fence.",
                },
    ...
    try:
        ...
        print(
            f"⚠️ aetherforge gateway unreachable: {type(e).__name__}: {str(e)[:120]}",
            file=sys.stderr,
        )
        return None
...
def main() -> int:
    ...
    p.add_argument("--no-llm", action="store_true", help="跳过 LLM 调用, 用启发式 fallback")
    ...
    print()
    print("下一步 (任选其一):")
    print("  1. python bin/commit-assist.py --apply  # 应用 suggestion")
    print("  2. git commit -F .commit-suggestion      # 等价")
    print("  3. 手动 git commit -m '<your message>'    # 不用 LLM 建议")
    return 0

...
已完成从真实外部资产库 `awesome-design-md` 到 `forge` 的上下游接入，并把生成结果落到真实清单文件中，不扩展 L4 控制面，也不创建第二控制器。当前落地已经稳定到“可用、可验证、可再用”的状态，并继续往 `design_context → page_spec → render` 推进。
...
- `forge.design_renderer`：新增 `build_page_spec` / `render_page_spec`，把选中的采样资产转成页面蓝图 `page_spec` 与可渲染输出
- `forge/README.md`：补充设计资产命令说明，方便直接调用和接入
- `tests/test_design_asset_adapter.py`：覆盖扫描、清单生成、过滤、结构化 context、prompt 注入和 page-spec 渲染验证
...
验证结论：
- `cd /Users/xiamingxing/ws-ui-continue && python3 -m pytest tests/test_design_asset_adapter.py -q`
- 结果：`4 passed in 1.40s`
...
- `PYTHONPATH=src python3 -m forge.forge design-page --query claude --format json`
- 结果：成功落成 `page_spec`，并输出 JSON 渲染结构

真实接入状态：
- 本地已完成真实资产连接、prompt 适配与 page-spec 桥接，不再依赖假样例 `stripe`
- 资产层仍保持只读输入，不写回 upstream repo，也不接管 omostation 主控制面
- 当前链路具备“真实检索 → 真实匹配 → 结构化 context → page_spec → render”的闭环

下一步：
- 可继续把 `page_spec` 挂到更高层 HTML / CSS / 渲染器，形成完整页面草图生成闭环

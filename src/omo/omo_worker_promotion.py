...
def _route_self_evolution_packet_to_remediation(
    root: Path,
    task_id: str,
    *,
    actor: str,
    ...
    review_note_body = (
        "\n".join(
            [
                f"# Self-Evolution Remediation Review — {task_id}",
                "",
                f"- Routed by: {actor}",
                f"- Routed at: {now or _utc_now()}",
    ...
    route_self_evolution_to_remediation(
        ...
        task_id=task_id,
        actor=actor,
        review_note_body=review_note_body,
...
def _write_task_approval_queue_status(
    ...
    for path in task_paths:
        ...
        try:
            entry = _promotion_approval_status_entry(root, path, omo_dir=omo_dir)
        except ValueError:
            # approval missing — queue 收集等 approval 的 task (不 reject)
            entry = {
                "task_id": task["id"],
        ...
        approval_status = str(entry.get("approval_status") or "missing")
        proposal_status = str(entry.get("proposal_status") or "missing")
        if blockers:
            next_action = "blocked"
        elif approval_status == "missing":
            next_action = "request_approval"
        elif proposal_status == "missing":
            next_action = "governance_approve"
        elif proposal_status == "approved":
            next_action = "promote_apply"
        else:
            next_action = "governance_apply"
        entries.append(
            {
    ...
    packet = build_approval_queue_packet(generated_at=now or _utc_now(), tasks=entries)
    output_dir = omo / "workers" / "promotion" / "approval-queue"

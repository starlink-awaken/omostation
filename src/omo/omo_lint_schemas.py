"""P100 refactor: omo_lint schemas 子模块 (从 omo_lint.py 提取).

P88 R1 提取 doc-lifecycle 子模块 → 304L (P88 后 omo_lint.py 1257→1257L)
P99 ADR-0093 规划 P100-P103 4 步拆解:
  P100 schemas 拆 (485L) → 825L, <1500L god-module threshold
  P101 surfaces 拆
  P102 mutation_ledger 拆
  P103 yaml_bypass 拆

7 个 schemas 维度 (Round 15/21/29/30/32/34/37 P0):
  1. _check_module_append_has_schema (R15) — consumer .append() 必传 schema=
  2. _check_schema_registry_integrity (R21) — Z-suffix + 必填字段
  3. _check_all_schemas_exported (R29) — __all__ 完整
  4. _check_cross_module_srp (R30) — 7 consumer 互不依赖
  5. _check_dead_imports (R32) — import 但未用
  6. _check_sort_keys_default (R34+R37) — sort_keys=True 跨仓不变量
  7. cmd_lint_schemas — 6 规则汇总入口

模块级常量 (4 个, schemas 区段独占):
  - OMO_SRC: Path(__file__).resolve().parent
  - CONSUMER_MODULES: 7 consumer tuple
  - _CROSS_MODULE_SRP_ALLOWLIST: §13.3.3 规则 7 白名单
  - _SORT_KEYS_DEFAULT_EXEMPT_MODULES: §12.1.4 跨仓豁免

向后兼容 (P88 模式):
  omo_lint.py 通过 `from .omo_lint_schemas import (...)` re-export,
  保持 `from omo.omo_lint import cmd_lint_schemas, _check_*, _OMO_*` 不破.
"""

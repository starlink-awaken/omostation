...
def check_all():
    ...
    l0_path = Path(__file__).resolve().parent.parent.parent / "protocols" / "L0-registry.yaml"
    import yaml
    ...
    print("\nL1 ── Runtime Matrix")
    services = []
    for name, port in [("Agora", 7430), ("Agent Runtime", 9876), ("Cron Service", 7450)]:
    ...
    ev_ok = _http_ok("http://127.0.0.1:7430/api/events")
    results.append(_fmt(ev_ok, "Agora Events :7430/api/events"))
    ...
    # L3: TaskObject
    print("\nL3 ── Entry Bridge")
    adapter = Path(__file__).parent / "taskobject_adapter.py"
    results.append(_fmt(adapter.exists(), f"TaskObject adapter: exists"))

    # KEI
    print("\nKEI ── Kernel Extension")
    audit = Path.home() / "runtime/audit/kei-audit.jsonl"
    if audit.exists():
        lines = len(audit.read_text().strip().split("\n"))
        results.append(_fmt(True, f"KEI audit: {lines} records"))
    else:
        results.append(_fmt(False, "KEI audit: file not found"))

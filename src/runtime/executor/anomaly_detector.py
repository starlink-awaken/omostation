...
RuleOperator = Literal[
    "eq", "ne", "gt", "gte", "lt", "lte", "contains", "matches", "in", "not_in"
]
...
class AnomalyRule:
    ...
    description: str
    status: str
    category: str  # noqa: E702
    condition: RuleCondition
    frequency: str
    severity: Severity  # noqa: E702
    intervention_action: InterventionAction
    requires_confirmation: bool
    cooldown_ms: float  # noqa: E702
    window_size: int | None = None
    burst_threshold: int | None = None  # noqa: E702
...
class DetectionResult:
    ...
    max_severity: Severity | Literal["none"]
    action: InterventionAction
    summary: str
    timestamp: float  # noqa: E702
...
class AnomalyRuleEngine:
    ...
    def evaluate(self, metrics: dict[str, Any]) -> DetectionResult:
        ...
        for rule in self.get_active_rules():
            ...
            matched = self._compare(
                actual, rule.condition.operator, rule.condition.value
            )
            if matched:
                ...
                triggered.append(
                    RuleResult(
                        ...
                        rule_name=rule.name,
                        triggered=True,
                        score=rule.condition.weight,
                        message=f"Triggered: {rule.description}",
                        timestamp=now,
                    )
                )
        ...
        for t in triggered:
            ...
            if (
                r and SEVERITY.get(r.severity, 0) > SEVERITY.get(sev, 0)
                if isinstance(sev, str) or SEVERITY.get(sev, 0)
                else False
            ):
        ...
        for t in triggered:
            r = self._rules.get(t.rule_id)
            if r and ACTION_PRI.get(r.intervention_action, 0) > ACTION_PRI.get(
                action, 0
            ):
                action = r.intervention_action
        return DetectionResult(
            has_anomaly=len(triggered) > 0,
            triggered=triggered,
            max_severity=sev,
            action=action,
            summary=f"{len(triggered)} anomaly(ies)" if triggered else "Normal",
            timestamp=now,
    ...
    def _compare(actual: Any, op: RuleOperator, expected: Any) -> bool:
        if op == "eq":
            return actual == expected  # noqa: E701
        if op == "ne":
            return actual != expected  # noqa: E701
        if op == "gt":
            return (
        ...
        if op == "matches":
            return (
                isinstance(actual, str)
                and isinstance(expected, str)
                and bool(re.search(expected, actual))
            )  # noqa: E701
        if op == "in":
            return isinstance(expected, list) and actual in expected  # noqa: E701
        if op == "not_in":
            return isinstance(expected, list) and actual not in expected  # noqa: E701
        return False

...
class IdentityManager:
    ...
    def __init__(self) -> None:
        ...
        self._reverse_dns_re = re.compile(
    ...
    def declare(self, agent: AgentDefinition) -> AgentIdentity:
        ...
        name = agent.name
        agent_id = (
            f"agent://{name.lower().replace(' ', '-')}"
            if name
            else self._generate_id(name)
        )
        identity = AgentIdentity(
            ...
            capabilities=[
                AgentCapability(id=c, description=c) for c in (agent.capabilities or [])
            ],
        )
    ...
    def validate(self, identity: AgentIdentity) -> IdentityValidationResult:
        ...
        if not identity.id:
            ...
            errors.append(
                f'A1.2: identity.id "{identity.id}" is not valid reverse-DNS format'
            )
        ...
        if identity.sovereignty_level.value not in valid_levels:
            errors.append(
                f"A1.5: sovereigntyLevel must be one of: {', '.join(sorted(valid_levels))}"
            )
        ...
        if not identity.capabilities:
            ...
            for i, cap in enumerate(identity.capabilities):
                ...
                if not cap.description:
                    warnings.append(
                        f'A1.6: capabilities[{i}] ("{cap.id or ""}") missing description'
                    )

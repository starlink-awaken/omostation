...
class ContractViolation:
    ...
    message: str  # noqa: E702


@dataclass
class ContractValidationResult:
    contract_id: str
    valid: bool  # noqa: E702
...
class ContractManager:
    ...
    def list_contracts(
        ...
        return (
            ...
            if status is None
            else [c for c in self._contracts.values() if c.status == status]
        )

    def validate_payload(
        self, cid: str, payload: dict[str, Any]
    ) -> ContractValidationResult:
        c = self._get(cid)
        errors, warnings = [], []  # noqa: E702
        for f in c.fields:
                ...
                if actual != f.type:
                    errors.append(
                        ContractViolation(
                            ...
                            actual=actual,
                            severity="error",
                            message=f"Expected {f.type} got {actual}",
                        )
                    )
    ...
    def compare_versions(
        self, cid: str, new_fields: list[ContractField]
            ...
            if name not in new_map:
                changes.append(
                    ContractChange(
                        type="removed",
                        field=name,
                        details=f"'{name}' removed",
                        breaking=cf.required,
                    )
                )
            ...
            if cf is None:
                changes.append(
                    ContractChange(
                        type="added",
                        field=name,
                        details=f"'{name}' added",
                        breaking=nf.required,
                    )
                )
                ...
                changes.append(
                    ContractChange(
                        type="modified",
                        field=name,
                        details=f"'{name}' changed",
                        breaking=cf.type != nf.type,
                    )
                )
        record = ContractChangeRecord(
            contract_id=cid,
    ...
    def get_consistency_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "consistency": 100
            if active + broken == 0
            else round(active / (active + broken) * 100),
        }

    def _get(self, cid: str) -> ContractDefinition:
        c = self._contracts.get(cid)
        if c is None:
            raise ValueError(f"Contract not found: {cid}")  # noqa: E701
        return c

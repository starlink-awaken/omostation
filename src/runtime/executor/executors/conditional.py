...
class IStepExecutor:
    async def execute_step(
        self, step: DSLNode, context: ExecutionContext
    ) -> ExecutionContext:
        raise NotImplementedError
...
class ConditionStepExecutor:
    ...
    def __init__(
        ...
        expression_evaluator: IExpressionEvaluator | None = None,
    ) -> None:
    ...
    async def execute_step(
        ...
        for branch in branches:
            if branch.else_call is not None:
                return await self._exec_call(
                    branch.else_call, resolved, outputs, step, enriched
                )  # type: ignore[arg-type]
            if (
                branch.if_expr
                and branch.then_call
                and self._is_truthy(self._expr_eval.evaluate(branch.if_expr, enriched))
            ):
                return await self._exec_call(
                    branch.then_call, resolved, outputs, step, enriched
                )  # type: ignore[arg-type]
        self._log_error(
            step, f"No matching branch in conditional step '{step_name}'", context
        )
        return ExecutionResult(
            success=False,
            value=None,
            trace=[],
            errors=[Exception(f"No matching branch in conditional step '{step_name}'")],
            outputs={},
        )
    ...
    def _enrich_context(
        self, context: ExecutionContext, resolved: dict[str, Any]
    ) -> ExecutionContext:
    ...
    async def _exec_call(
        ...
        target = (
            getattr(call, "name", step)
            if call_type in ("agent", "tool")
            else getattr(call, "skill_id", step)
        )
        temp_call = DSLCall(
            type=call_type,
            name=getattr(call, "name", ""),
            skill_id=getattr(call, "skill_id", ""),
        )
        temp_inputs = {k: DSLLiteral(value=v) for k, v in resolved.items()}
        temp_step = DSLStep(
            name=str(target), call=temp_call, inputs=temp_inputs, outputs=outputs
        )  # type: ignore[arg-type]
        try:
    ...
    def _log_error(
        self, step: DSLNode, message: str, context: ExecutionContext
    ) -> None:
...
class DefaultExpressionEvaluator(IExpressionEvaluator):
    ...
    def evaluate(self, expr: DSLExpression, context: ExecutionContext) -> Any:
        ...
        if expr.type == DSLNodeType.BINARY_OP:
            left, right = (
                self.evaluate(getattr(expr, "left"), context),
                self.evaluate(getattr(expr, "right"), context),
            )
            return self._bin_op(getattr(expr, "operator", ""), left, right, context)
        if expr.type == DSLNodeType.UNARY_OP:
            op, operand = (
                getattr(expr, "operator", ""),
                self.evaluate(getattr(expr, "operand"), context),
            )
            return {
                "!": lambda: not self._is_truthy(operand),
                "-": lambda: -operand,
                "+": lambda: +operand,
            }[op]()
        ...
        if expr.type == DSLNodeType.CONDITIONAL_EXPRESSION:
            ...
            return (
                self.evaluate(getattr(expr, "consequent"), context)
                if self._is_truthy(test)
                else self.evaluate(getattr(expr, "alternate"), context)
            )
    ...
    def _bin_op(self, op: str, left: Any, right: Any, ctx: ExecutionContext) -> Any:
        ...
        tables = {
            ...
            "<=": lambda: left <= right,
            ">=": lambda: left >= right,
            "&&": lambda: self._is_truthy(left) and self._is_truthy(right),
            "||": lambda: self._is_truthy(left) or self._is_truthy(right),
        }
        fn = tables.get(op)
        if fn is None:

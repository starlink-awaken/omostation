...
class ParallelExecutor:
    ...
    async def execute(
        self, stmt: DSLParallel, context: ExecutionContext
    ) -> ExecutionResult:
        ...
        if current_depth >= self._max_nesting:
            ...
            return ExecutionResult(
                success=False,
                value=None,
                trace=self._make_trace(stmt, start_time, "failure", errors),
                errors=[err],
            )

        max_concurrency = self._resolve_max_concurrency(stmt)
        results = await self._execute_branches(
            stmt.branches, context, max_concurrency, current_depth + 1
        )
        ...
        if self._enable_tracing:
            trace.append(
                TraceEntry(
                    timestamp=time.time(),
                    statement_type="parallel",
                    status="failure" if errors else "success",
                    data={
                        "branch_count": len(stmt.branches),
    ...
    async def _execute_branches(
        self,
        branches: list[list[DSLStatement]],
        ...
        async def _exec_branch(
            branch: list[DSLStatement], _index: int
        ) -> ExecutionResult:
            ctx = ExecutionContext(
                input=parent_ctx.input.copy(),
                locals={},
                results={},
        ...
        if max_concurrency is None or max_concurrency >= branch_count:
            settled = await asyncio.gather(
                *[_exec_branch(b, i) for i, b in enumerate(branches)],
            ...
            return [
                ...
                if isinstance(r, ExecutionResult)
                    ...
                    success=False, value=None, trace=[], errors=[Exception(str(r))]
                )
                for r in settled
            ]
    ...
    async def _execute_with_limit(
        self, branches: list[list[DSLStatement]], exec_fn: Any, limit: int
    ) -> list[ExecutionResult]:
        results: list[ExecutionResult] = [
            ExecutionResult(success=True, value=None, trace=[], errors=[])
            for _ in branches
        ]
    ...
    def _make_trace(
        self, stmt: DSLParallel, start_time: float, status: str, errors: list[Exception]
    ) -> list[TraceEntry]:
        return [
            TraceEntry(
                ...
                data={
                    "branch_count": len(stmt.branches),
                    "max_concurrency": stmt.max_concurrency,

...
class TryCatchExecutor:
    ...
    async def execute(
        self, stmt: DSLTryCatch, context: ExecutionContext
    ) -> ExecutionResult:
        ...
        try:
            ...
            if self._enable_tracing:
                trace.append(
                    TraceEntry(
                        timestamp=time.time(),
                        statement_type="try_catch",
                        status="failure",
                        data={
                            "phase": "try",
                            "has_catch": bool(stmt.catch_block),
                            "has_finally": bool(stmt.finally_block),
                        },
                    )
                )
        ...
        if stmt.catch_block:
            ...
            for catch_stmt in stmt.catch_block:
                try:
                    result = await self._stmt_exec.execute_statement(
                        catch_stmt, child_ctx
                    )
                    trace.extend(getattr(result, "trace", []) or [])
                    errors.extend(getattr(result, "errors", []) or [])
                except Exception as exc:  # noqa: BLE001  # defensive fallback
        ...
        if self._enable_tracing:
            trace.append(
                TraceEntry(
                    ...
                    data={
                        "caught": caught_error is not None,
                        "has_catch": bool(stmt.catch_block),
                        "has_finally": bool(stmt.finally_block),
                    },
                )
        ...
        return ExecutionResult(
            success=success, value=None, trace=trace, errors=errors if errors else []
        )

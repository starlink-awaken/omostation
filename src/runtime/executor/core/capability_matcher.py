...
def _calculate_schema_compatibility(
    request: CapabilityMatchRequest, cap: CapabilityDefinition
) -> float:
    score = 1.0
...
class CapabilityMatcher:
    ...
    def _match_exact(
        self, request: CapabilityMatchRequest
        ...
        for dc in self._caps:
            ...
            if request.type is not None and cap.type.value != request.type:
                continue
            if request.min_level and not meets_capability_level(
                cap.level, request.min_level
            ):
                continue
            if (
                request.max_duration_ms is not None
                and cap.performance
                and cap.performance.avg_duration_ms is not None
            ):
            ...
            if (
                ...
                and cap.performance
                and cap.performance.avg_tokens is not None
            ):
            ...
            if request.tags:
                ...
                if not all(t in cap_tags for t in request.tags):
                    continue
            if request.output_schema and not _check_output_compatibility(
                cap.output, request.output_schema
            ):
                continue
            reasons = [f"Type matches: {cap.type.value}"]
            if request.min_level:
                reasons.append(
                    f"Level {cap.level.value} meets {request.min_level.value}"
                )
            results.append(
                CapabilityMatchResult(
                    agent_name=dc.agent_name, capability=cap, score=1.0, reasons=reasons
                )
            )
        ...
        for dc in self._caps:
            ...
            if request.min_level:
                ...
                if meets_capability_level(cap.level, request.min_level):
                    diff = abs(
                        _LEVEL_ORDER[cap.level.value]
                        - _LEVEL_ORDER[request.min_level.value]
                    )
    ...
    def _match_any(
        ...
        for dc in self._caps:
            ...
            if request.min_level and meets_capability_level(
                cap.level, request.min_level
            ):
                score += 0.1
    ...
    def find_top_matches(
        self, request: CapabilityMatchRequest, n: int
    ) -> list[CapabilityMatchResult]:
        return self.match(request)[:n]

    def update_discovered_capabilities(
        self, discovered: list[DiscoveredCapability]
    ) -> None:
        self._caps = discovered


def create_capability_matcher(
    discovered: list[DiscoveredCapability], weights: MatchWeights | None = None
) -> CapabilityMatcher:

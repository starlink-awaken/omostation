...
_LTYPES = {
    "L1": _KWT.ANALYSIS,
    "L2": _KWT.COORDINATION,
    "L3": _KWT.EXECUTION,
    "L4": _KWT.VALIDATION,
    "governance": _KWT.MONITORING,
}
...
class CapabilityDiscovery:
            ...
            if (full := f"{name}-{cid}") not in seen:
                ...
                defs.append(
                    CapabilityDefinition(
                        id=full,
                        type=infer_capability_type(cid, layer),
                        name=cid,
                        description=f"Capability to {cid} ({name})",
        ...
        for tool in agent.get("tools") or []:
            if (full := f"{name}-{tool}") not in seen:
                ...
                defs.append(
                    CapabilityDefinition(
                        id=full,
                        type=infer_capability_type(tool, layer),
                        name=tool,
    ...
    def find_by_tags(
        ...
        if match_all:
            return [
                dc
                for dc in self._by_id.values()
                if all(t in (dc.capability.tags or []) for t in tags)
            ]
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        for dc in caps:
            by_type[dc.capability.type.value] = (
                by_type.get(dc.capability.type.value, 0) + 1
            )
            by_level[dc.capability.level.value] = (
                by_level.get(dc.capability.level.value, 0) + 1
            )
        ...
        return {
            ...
            "capabilities_by_level": by_level,
            "capabilities_by_agent": by_agent,
    ...
    def get_cache_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "age": age,
            "capability_count": len(self._by_id),

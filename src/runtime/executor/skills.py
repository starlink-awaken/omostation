...
def create_builtin_skills() -> SkillRegistry:
    ...
    registry.register(
        "summarize",
        lambda ctx: f"Summary of: {ctx.get('text', '')[:100]}",
        "Summarize text content",
        "documentation",
    )
    registry.register(
        "code_review",
        lambda ctx: {"issues": [], "suggestions": [], "score": 5.0},
        "Review code for issues and suggest improvements",
        "code",
    )
    registry.register(
        "test_generate",
        lambda ctx: {"tests": [], "coverage_estimate": 0.0},
    ...
    registry.register(
        "format_output",
        lambda ctx: str(ctx.get("data", "")),
        "Format data as string output",
        "formatting",
    )
    return registry

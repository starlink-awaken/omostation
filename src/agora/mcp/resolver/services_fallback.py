"""agora.mcp.resolver.services_fallback — fallback services 拆分 (P111).

TASK-F7114ABA 治本: services.py 1511L God Module, 将 _FALLBACK_SERVICES
拆到本模块。services.py 降至 <100L。

模式: 顶层 re-export (PFC) 保持 from .services import _FALLBACK_SERVICES 不破。
调用方 from agora.mcp.resolver.services import _FALLBACK_SERVICES 等。
"""
...
_FALLBACK_SERVICES: list[BosService] = [
    ...
    BosService(
        ...
        description="AetherForge 算力网关与 omlxc (Apple MLX/Local LLM) 本地量化模型通用推理调度端点",
    ),
    BosService(
        uri="bos://compute/aetherforge/mesh",
        domain="compute",
        package="aetherforge",
        action="mesh",
        transport="stdio",
        ...
        description="AetherForge 边缘多节点网格状态与拓扑健康度查询端点 (oMLXC Compute Mesh)",
    ),
    BosService(
        ...
        description="AetherForge 硬件性能特征剖析与模型量化适配检查端点 (Hardware Profile)",
    ),
    BosService(
        ...
        description="omlxc Compute Fabric 异构算力拓扑、硬件温控、显存预算与分诊状态诊断端点",
    ),
    BosService(
        ...
        description="omlxc Intent Triage 意图复杂度轻量初筛与模型等级决策端点",
    ),
    BosService(

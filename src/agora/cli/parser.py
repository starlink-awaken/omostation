...
def run_proxy_launch(args):
    ...
    if args.status:
        ...
        print()
        print(
            f"{'Service':<22} {'Source':<10} {'Avail':<8} {'Installed':<10} Description"
        )
        print("-" * 80)
        for svc in status["services"]:
            ...
            print(
                f"{svc['name']:<22} {svc['source']:<10} {exists:<8} {installed:<10} {svc['description']}"
            )
        return
...
For more: https://github.com/starlink-awaken/agora#readme""",
    ...
    r.add_argument(
        "--protocol",
        default="mcp",
        choices=sorted(KNOWN_PROTOCOLS),
        help="Service protocol (default: mcp)",
    )
    r.add_argument(
        "--protocol-config", default="{}", help="Protocol-specific config as JSON"
    )
    r.add_argument(
        "--mcp", default="", help="MCP endpoint URL (or generic endpoint for non-MCP)"
    )
    ...
    r.add_argument(
        "--has-auth", action="store_true", help="Service uses authentication"
    )
    r.add_argument(
        "--has-push-notifications",
        action="store_true",
        help="Service supports push notifications",
    )
    r.add_argument(
        "--has-state-transitions",
        action="store_true",
        help="Service tracks state transitions",
    )
    r.add_argument(
        "--provider-info",
        default="",
        help='Provider info JSON (e.g. \'{"organization":"MyOrg"}\')',
    )
    r.add_argument(
        "--documentation-url", default="", help="Documentation URL for the service"
    )
    ...
    d.add_argument(
        "--register", action="store_true", help="Auto-register discovered services"
    )
    d.add_argument("--json", action="store_true", help="JSON output")
    d.add_argument(
        "--watch", action="store_true", help="Watch mode: continuous discovery"
    )
    d.add_argument(
        "--interval",
        type=int,
    ...
    d.add_argument(
        "--probe", action="store_true", help="Enable port probing (async, slow)"
    )
    ...
    inst_register = inst_sub.add_parser(
        "register", help="Register this instance with the peer network"
    )
    inst_register.add_argument("instance_id", help="Unique instance ID")
    inst_register.add_argument(
        "--type", default="mcp-server", help="Instance type (default: mcp-server)"
    )
    inst_register.add_argument(
        "--display-name", default="", help="Human-readable display name"
    )
    ...
    inst_register.add_argument(
        "--capabilities", default="", help="Comma-separated capabilities"
    )
    ...
    ten_add.add_argument(
        "--services", default="", help="Comma-separated allowed services"
    )
    ten_add.add_argument(
        "--rate-limit", type=int, default=60, help="Requests per minute"
    )
    ...
    mkt_install.add_argument(
        "name",
        help="Service name or GitHub repo (e.g. filesystem, starlink-awaken/minerva)",
    )
    mkt_pub = mkt_sub.add_parser("publish", help="Publish a service to the market")
    mkt_pub.add_argument("name", help="Service name")
    mkt_pub.add_argument(
        "--repo", default="", help="GitHub repo (e.g. starlink-awaken/my-service)"
    )
    ...
    repo_list.add_argument(
        "--status",
        choices=["discovered", "installed", "loaded", "idle"],
    ...
    repo_discover = repo_sub.add_parser(
        "discover", help="Discover tools from external sources"
    )
    repo_discover.add_argument(
        "query",
        nargs="?",
        default="mcp-server",
        help="Search query (default: mcp-server)",
    )
    ...
    repo_load = repo_sub.add_parser(
        "load", help="Load a tool via LifecycleManager (Phase 2)"
    )
    repo_load.add_argument("name_or_id", help="Tool name or ID to load")

    repo_unload = repo_sub.add_parser(
        "unload", help="Unload a tool via LifecycleManager (Phase 2)"
    )
    ...
    repo_pipeline = repo_sub.add_parser(
        "pipeline", help="Full discover → install → load pipeline (Phase 2)"
    )
    repo_pipeline.add_argument(
        "query",
        nargs="?",
        default="mcp-server",
        help="Search query (default: mcp-server)",
    ...
    repo_run = repo_sub.add_parser(
        "run", help="Execute a natural language query via Smart Router"
    )
    ...
    sync_parser = sub.add_parser(
        "sync", help="Sync workspace services to Agora registry"
    )
    ...
    converge_parser = sub.add_parser(
        "converge", help="Converge workspace services (delegates to sync)"
    )
    ...
    # proxy-launch
    proxy_launch = sub.add_parser(
        "proxy-launch", help="Launch downstream MCP services via auto-bootstrap"
    )
    proxy_launch.add_argument(
        "--status", action="store_true", help="Show service status without launching"
    )
    proxy_launch.add_argument(
        "--reload", action="store_true", help="Regenerate config from KNOWN_SERVICES"
    )
    proxy_launch.add_argument(
        "--edit", action="store_true", help="Open config file in editor"
    )
    ...
    start_parser.add_argument(
        "--port",
        type=int,
        default=int(os.environ.get("AGORA_MCP_HTTP_PORT", "7422")),
        help="Port number (default: AGORA_MCP_HTTP_PORT)",
    )
    start_parser.set_defaults(func=start_pipeline_command)

    unified_parser = sub.add_parser(
        "run", help="Unified entry -- auto-route to the right tool"
    )
    ...
    acct_top.add_argument(
        "--limit", type=int, default=10, help="Number of top callers (default: 10)"
    )
    ...
    acct_quota.add_argument(
        "--caller", required=True, help="Caller ID to check quota for"
    )
    acct_quota.add_argument(
        "--quota", type=float, default=10.0, help="Daily quota in USD (default: 10.0)"
    )
    ...
    pl.add_argument(
        "--stream", action="store_true", help="Stream each step as it completes"
    )
    pl.add_argument(
        "--parallel", action="store_true", help="Execute independent steps concurrently"
    )
    ...
    key_create.add_argument(
        "--scopes", default="read", help="Comma-separated scopes (read,write,admin)"
    )
    key_create.add_argument("--tenant", default="", help="Tenant name")
    key_create.add_argument(
        "--expires", type=int, default=0, help="Expiry in days (0=never)"
    )
    ...
    key_rotate = key_sub.add_parser(
        "rotate", help="Rotate an API key (revoke old, create new with same attributes)"
    )
    key_rotate.add_argument("key_id", help="Key ID to rotate")
    key_rotate.add_argument(
        "--expires", type=int, default=0, help="New expiry in days (0=keep same)"
    )
    ...
    a2a_list.add_argument(
        "--status",
        default="",
        help="Filter by status (submitted|working|completed|failed|canceled)",
    )
    ...
    proto_compile = proto_sub.add_parser(
        "compile", help="Compile .proto to pb2_grpc.py"
    )
    proto_compile.add_argument("proto_file", help="Path to .proto file")
    proto_compile.add_argument(
    ...
    # pipeline-define
    pd = sub.add_parser(
        "pipeline-define", help="Define a custom pipeline from JSON file"
    )
    ...
    enf_set.add_argument(
        "tool", nargs="+", default=[], help="Tool patterns (e.g. collab.* or *)"
    )
    ...
    identity_issue.add_argument(
        "--subject-type", default="service", help="Subject type (default: service)"
    )
    ...
    grant_create.add_argument(
        "capability", help="Capability name (e.g. service:invoke)"
    )
...
def start_pipeline_command(args):
    ...
    if args.pipeline not in EIDOS_PIPELINE_SERVICE.get("commands", {}):
        ...
        print(
            f"Available: {list(EIDOS_PIPELINE_SERVICE.get('commands', {}).keys())}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    class PipelineHandler(BaseHTTPRequestHandler):
        def do_GET(self):
            if self.path == "/tools":
                ...
                self.wfile.write(
                    json.dumps(
                        {"pipelines": list(EIDOS_PIPELINE_SERVICE["commands"].keys())}
                    ).encode()
                )
            else:

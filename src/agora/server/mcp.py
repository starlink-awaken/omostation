...
_AUDIT_DB = Path(os.environ.get("AGORA_AUDIT_DB", str(get_data_dir() / "agora-audit.db")))


class AuditSubscriber:
    """Subscribes to all EventBus events and persists them for audit."""

    def __init__(self, event_bus: EventBus, registry: ServiceRegistry | None = None, db_path: str | Path | None = None):
        self._bus = event_bus
        self._registry = registry
        self._db_path = Path(db_path or _AUDIT_DB)
        self._init_db()
    ...
    def _classify(self, event_type: str) -> dict:
        ...
        return {"actor": info["actor"], "resource": info["resource"], "action": verb, "risk_level": info["risk"]}
    ...
    def on_event(self, event: dict):
        ...
        try:
            ...
            conn.execute(
                ...
                (f"{event_id}", ts, event_type, source, classified["actor"], classified["resource"],
                 classified["action"], trace_id, payload_str, classified["risk_level"], payload.get("_duration_ms", 0.0)),
            )
            conn.commit()
            conn.close()
    ...
    def query(self, actor: str = "", resource: str = "", event_type: str = "", since: str = "", limit: int = 50) -> list[dict]:
        ...
        try:
            ...
            rows = conn.execute(f"SELECT * FROM audit_log WHERE {where} ORDER BY timestamp DESC LIMIT ?", [*params, limit]).fetchall()
            conn.close()
            result = []
            for row in rows:
                entry = dict(row)
    ...
    def stats(self, since: str = "") -> dict:
        stats: dict = {"total": 0, "by_risk": {}, "by_event_type": {}}
        try:
            conn = sqlite3.connect(str(self._db_path))
            if since:
                rows = conn.execute("SELECT risk_level, COUNT(*) as cnt FROM audit_log WHERE timestamp >= ? GROUP BY risk_level", (since,)).fetchall()
            ...
            type_rows = conn.execute("SELECT event_type, COUNT(*) as cnt FROM audit_log GROUP BY event_type ORDER BY cnt DESC LIMIT 20").fetchall()
    ...
    def to_otel_json(self, since: str = "") -> list[dict]:
        ...
        for entry in entries:
            ...
            spans.append({
                ...
                "traceId": trace_hex, "spanId": span_id_hex,

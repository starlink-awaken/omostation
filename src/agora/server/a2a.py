    ...
    try:
        ...
        if signature:
            ...
            if abs(now - msg_ts) > 300:
                logger.warning("a2a_replay_blocked: timestamp_out_of_window", extra={"node_id": sender_node_id, "ts": msg_ts})
                return JSONResponse({"status": "error", "error": "timestamp_out_of_window"}, status_code=401)
            
            # ── Phase 15: Replay Check ──
            if signature in _SEEN_SIGNATURES:
                logger.warning("a2a_replay_blocked: signature_already_seen", extra={"node_id": sender_node_id})
                return JSONResponse({"status": "error", "error": "replay_detected"}, status_code=401)

...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Forge design asset discovery, context building, and page-spec rendering.")
    ...
    render_parser = subparsers.add_parser("design-page", help="Render a selected design asset into a page spec")
    render_parser.add_argument("repo_path", nargs="?", default=str(find_awesome_design_repo()))
    render_parser.add_argument("--query", default="claude")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "design-page":
        ...
        if not assets:
            print(json.dumps({"error": "No matching design assets found."}, ensure_ascii=False, indent=2))
            return 1
        spec = build_page_spec(assets[0], query=args.query)
        rendered = render_page_spec(spec, output_format=args.format)
        print(rendered)
        return 0
    parser.print_help()
    return 1

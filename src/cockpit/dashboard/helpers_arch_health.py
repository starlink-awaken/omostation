"""cockpit.dashboard.helpers_arch_health — load_arch_health 拆分 (P110-E).

TASK-F7114ABA (omo lint god-module 800L 硬规则).
cockpit/dashboard/helpers.py 1116L 拆分: load_arch_health (~205L) 独立到本模块.

业务: 聚合 architecture health metrics (test baseline + e2e + omo report +
bos metrics + debt 概览).

模式: load_arch_health 内部 import 共享 helper (lazy import 避免循环).
调用方 `from cockpit.dashboard.helpers import load_arch_health` 不破.
"""
...
def load_arch_health() -> dict:
    ...
    from cockpit.dashboard.helpers import parse_timestamp

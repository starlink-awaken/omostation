...
Usage:
    ...
    register_subcommands(sub_parser, WorkspaceParser)
"""
...
def register_subcommands(sub: argparse._SubParsersAction, WorkspaceParser: type[argparse.ArgumentParser]) -> None:
    """Register all cockpit subcommands on the given sub-parser.

    Args:
        sub: The subparsers action from parser.add_subparsers()
        WorkspaceParser: Custom ArgumentParser class for workspace-aware help/errors
    """
    ...
    r.add_argument(
        "--style", choices=["brief", "report", "memo"], default="report", help="publish 的输出风格"
    )
    ...
    r.add_argument(
        "--archive", type=int, nargs="+", metavar="ID", help="归档研究记录"
    )
    r.add_argument(
        "--unarchive", type=int, nargs="+", metavar="ID", help="恢复已归档研究记录"
    )
    r.add_argument(
        "--all-active",
        action="store_true",
    ...
    r.add_argument(
        "--compare", type=int, nargs="+", metavar="ID", help="对比多个研究结果"
    )
    r.add_argument(
        "--merge", type=int, nargs="+", metavar="ID", help="合并多个研究结果为新研究"
    )
    r.add_argument(
        "--digest", type=int, nargs="+", metavar="ID", help="提炼多个研究结果为 digest"
    )
    r.add_argument("--audit", action="store_true", help="扫描可疑研究记录")
    r.add_argument(
        "--quarantine", type=int, nargs="+", metavar="ID", help="隔离可疑研究记录"
    )
    r.add_argument(
        "--restore", type=int, nargs="+", metavar="ID", help="恢复已隔离研究记录"
    )
    ...
    r.add_argument(
        "--agent", type=str, metavar="NAME", help="标记/查询处理 Agent (如 minerva, sophia)"
    )
    r.add_argument("--heatmap", action="store_true", help="显示研究活跃度热力图")
    r.add_argument(
        "--follow-up", action="store_true", help="查看追问工作台（待追问/已回答统计）"
    )
    r.add_argument(
        "--health", action="store_true", help="查看研究健康报告（衰减状态/保鲜建议）"
    )
    r.add_argument(
        "--batch", action="store_true", help="批量研究模式: 逐个处理多个 topic，汇总结果"
    )
    ...
    r.add_argument(
        "--backup-restore", type=str, metavar="PATH", help="从备份 JSON 文件恢复研究数据"
    )
    r.add_argument(
        "--json", action="store_true", help="以 JSON 格式输出（--list 和 --open 模式可用）"
    )
    ...
    status_p.add_argument(
        "--interval", type=float, default=5.0, help="监控刷新间隔（秒）"
    )
    ...
    debt_score_p.add_argument(
        "impact", type=int, nargs="?", default=5, help="影响 (1-10)"
    )
    debt_score_p.add_argument(
        "frequency", type=int, nargs="?", default=5, help="频率 (1-10)"
    )
    debt_score_p.add_argument(
        "cost", type=int, nargs="?", default=5, help="修复成本 (1-10)"
    )
    debt_score_p.add_argument("--stage", default="stable_growth", help="项目阶段")
    debt_score_p.add_argument(
        "--list-stages", action="store_true", help="列出可用阶段"
    )
    ...
    debt_list_p.add_argument(
        "omo_debt_args", nargs=argparse.REMAINDER, help="传给 omo debt 的参数"
    )
    debt_summary_p = debt_sub.add_parser("summary", help="债务摘要 (委派 omo debt)")
    debt_summary_p.add_argument(
        "omo_debt_args", nargs=argparse.REMAINDER, help="传给 omo debt 的参数"
    )
    ...
    sub.add_parser(
        "gac", help="GaC 治理健康检查 (ADR-0106, 7 机制 + 115 规则 + drift)"
    )
    ...
    data_index_p.add_argument(
        "--json", action="store_true", help="以 JSON 输出索引结果"
    )
    data_types_p = data_sub.add_parser("types", help="查看已注册的数据类型")
    data_types_p.add_argument("--root", help="显式指定 workspace root")
    data_types_p.add_argument(
        "--json", action="store_true", help="以 JSON 输出类型注册表"
    )
    ...
    data_gc_p.add_argument(
        "--max-age-hours", type=float, default=24.0, help="TTL 小时数（默认 24）"
    )
    data_gc_p.add_argument("--json", action="store_true", help="以 JSON 输出清理结果")

    # ── contracts ─────────────────────────────────────────────
    contracts_p = sub.add_parser("contracts", help="契约验证")
    contracts_sub = contracts_p.add_subparsers(
        dest="contracts_command", parser_class=WorkspaceParser
    )
    validate_p = contracts_sub.add_parser("validate", help="验证 Workspace 契约")
    validate_p.add_argument(
        "path", nargs="?", help="可选：要验证的 WorkspaceObject JSON 文件"
    )
    contracts_sub.add_parser("list", help="列出所有已注册的 Schema")
    export_research_p = contracts_sub.add_parser(
        "export-research", help="将研究对象导出为 WorkspaceObject JSON"
    )
    export_research_p.add_argument(
        "research_id", type=int, metavar="ID", help="研究对象 ID"
    )
    export_research_p.add_argument(
        "--output", "-o", help="写入目标 JSON 文件；不提供则打印到 stdout"
    )
    export_p = contracts_sub.add_parser("export", help="导出契约封套")
    export_sub = export_p.add_subparsers(dest="contracts_export_type")
    export_id_p = export_sub.add_parser(
        "identity", help="导出身份封套 (IdentityEnvelope)"
    )
    export_id_p.add_argument("--output", "-o", help="写入目标文件")
    export_event_p = export_sub.add_parser(
        "event", help="导出事件封套 (EventEnvelope)"
    )
    export_event_p.add_argument(
        "--id", type=int, help="研究对象 ID 以导出其事件"
    )
    ...
    qs_p = sub.add_parser(
        "quickstart", help="🚀 新用户快速上手向导（环境核验 + 上手指引）"
    )
    qs_p.add_argument(
        "--fix", action="store_true", help="自动检测并修复常见问题"
    )
    qs_p.add_argument(
        "--model",
        default="llama3.2",
    ...
    audit_p = sub.add_parser(
        "audit", help="🔍 6 维度全方位审计 (调 bin/workspace-audit)"
    )
    ...
    audit_p.add_argument(
        "--output", type=str, default=None, help="写报告到文件"
    )
    audit_p.add_argument(
        "--since", type=str, default="7d", help="agora 维度时间范围 (默认 7d)"
    )
    ...
    mcp_p.add_argument(
        "--list-tools", action="store_true", help="列出已注册的工具，不启动 server"
    )
    mcp_p.add_argument(
        "--agora",
        action="store_true",
    ...
    cards_p.add_argument(
        "--check", action="store_true", help="检查当前操作合规性"
    )
    ...
    skill_p.add_argument(
        "skill_name", help="技能名称 (如 kos-daily-ontology-sync)"
    )
    ...
    brief_p.add_argument(
        "--force", action="store_true", help="强制重新生成"
    )

    search_p = sub.add_parser(
        "search", help="跨源搜索 (数据库 + BOS 知识引擎)"
    )
    ...
    search_p.add_argument(
        "--json", action="store_true", help="输出 P2 memory spine 统一 JSON 格式"
    )
    search_p.add_argument(
        "--limit", type=int, default=10, help="每源结果数 (默认10)"
    )

    sub.add_parser("discover", help="发现可用功能和资源")

    events_p = sub.add_parser(
        "events", help="实时查看 Agora SSE 事件流 (Phase 34 L3 Dashboard)"
    )
    ...
    tui_p = sub.add_parser(
        "tui", help="极客终端交互控制台 (Textual 全屏 TUI)"
    )
    tui_p.add_argument(
        "--theme", default="dark", choices=["dark", "light"], help="配色主题"
    )

    # ── BOS capability / inbox / watch ────────────────────────
    bcap_p = sub.add_parser(
        "bos-capability", help="BOS capability / toolbox 外部能力"
    )
    bcap_p.add_argument(
        "capability_command",
        nargs="?",
    ...
    binbox_p = sub.add_parser(
        "bos-inbox", help="BOS Inbox 多源私有知识神经网查询与操作"
    )
    ...
    ewatch_p = sub.add_parser(
        "events-watch", help="实时监听 SSE 事件流简便入口"
    )
    ewatch_p.add_argument(
        "--limit", type=int, default=20, help="显示最近的事件条数"
    )
    qcheck_p = sub.add_parser(
        "quickstart-check", help="快速检查新用户环境核验状态"
    )
    qcheck_p.add_argument(
    ...
    ssb_p.add_argument(
        "extra", nargs=argparse.REMAINDER, help="传递给 ecos-ssb 的参数"
    )
    ...
    mof_p.add_argument(
        "extra", nargs=argparse.REMAINDER, help="传递给 mof 的参数"
    )
    ...
    brain_sub = brain_p.add_subparsers(
        dest="brain_subcommand", parser_class=WorkspaceParser
    )
    brain_ask_p = brain_sub.add_parser(
        "ask", help="向大脑提问（知识检索 + LLM 回答）"
    )
    brain_ask_p.add_argument(
        "question", nargs=argparse.REMAINDER, help="你的问题"
    )
    brain_sub.add_parser("context", help="查看当前记忆摘要")
    brain_remember_p = brain_sub.add_parser(
        "remember", help="手动存入偏好/事实"
    )
    brain_remember_p.add_argument(
        "fact", nargs=argparse.REMAINDER, help="要记住的内容"
    )
    brain_history_p = brain_sub.add_parser("history", help="查看对话历史")
    brain_history_p.add_argument(
        "--limit", "-n", type=int, default=20, help="显示条数"
    )
    ...
    bus_data_p = bus_sub.add_parser(
        "data", help="数据平面：emit 高吞吐 fire-and-forget"
    )
    ...
    bus_control_sub = bus_control_p.add_subparsers(
        dest="control_command", parser_class=WorkspaceParser
    )
    bus_control_submit_p = bus_control_sub.add_parser(
        "submit", help="提交控制任务"
    )
    bus_control_submit_p.add_argument("--topic", required=True, help="topic 名")
    bus_control_submit_p.add_argument(
        "--payload", default="{}", help="JSON payload"
    )
    bus_control_ack_p = bus_control_sub.add_parser(
        "ack", help="确认任务完成"
    )
    bus_control_ack_p.add_argument("--task-id", required=True, help="任务 ID")
    bus_control_nack_p = bus_control_sub.add_parser(
        "nack", help="否定确认任务"
    )
    bus_control_nack_p.add_argument("--task-id", required=True, help="任务 ID")
    bus_control_nack_p.add_argument(
        "--error", default="nacked", help="失败原因"
    )

    # ── observe / family-hub / mesh ───────────────────────────
    observe_p = sub.add_parser("observe", help="可观测性栈（Langfuse）入口")
    observe_sub = observe_p.add_subparsers(
        dest="observe_command", parser_class=WorkspaceParser
    )
    ...
    observe_logs_p.add_argument(
        "--service", default="langfuse-server", help="服务名"
    )
    ...
    bos_resolve_p = bos_sub.add_parser(
        "resolve", help="统一 BOS URI 路由解析与目标元数据提取"
    )
    bos_resolve_p.add_argument("uri", help="BOS URI, e.g. bos://memory/inbox/status")
    bos_read_p = bos_sub.add_parser(
        "read", help="通过 BOS 网关统一读取指定 URI 资源"
    )
    bos_read_p.add_argument("uri", help="BOS URI, e.g. bos://memory/inbox/status")
    bos_read_p.add_argument(
        "--args", default="{}", help="JSON 格式查询参数字符串"
    )
    ...
    bos_mutate_p.add_argument(
        "--payload", default="{}", help="JSON 格式 payload"
    )
    bos_mutate_p.add_argument(
        "--action",
        default="update",
    ...
    bos_inbox_p = bos_sub.add_parser(
        "inbox", help="BOS Inbox 多源私有知识神经网查询与操作"
    )
    bos_inbox_sub = bos_inbox_p.add_subparsers(dest="inbox_cmd")
    bos_inbox_sub.add_parser(
        "status", help="统计多源神经网本地证据与嵌入状态"
    )
    bos_inbox_search_p = bos_inbox_sub.add_parser(
        "search", help="语义搜索多源记忆"
    )
    bos_inbox_search_p.add_argument("query", help="搜索关键词")
    bos_inbox_pending_p = bos_inbox_sub.add_parser(
        "pending", help="查看未决待办快照预览"
    )
    ...
    bos_inbox_archive_p = bos_inbox_sub.add_parser(
        "archive", help="归档已处理完毕的 Inbox 待办文件"
    )
    bos_inbox_archive_p.add_argument("filename", help="文件名或 all")
    bos_inbox_archive_p.add_argument(
        "--reason", default="resolved", help="归档事由"
    )

    bos_capability_p = bos_sub.add_parser(
        "capability", help="BOS capability 域 / toolbox 外部能力"
    )
    bos_capability_sub = bos_capability_p.add_subparsers(
        dest="capability_command"
    )
    bos_capability_sub.add_parser(
        "list", help="列出 toolbox 中的 capability 服务"
    )
    ...
    wf_p.add_argument(
        "workflow_args", nargs="*", help="workflow 子命令和参数"
    )
    ...
    onboard_p.add_argument(
        "--profile", help="Check if a specific profile is registered"
    )
    ...
    agent_runtime_p.add_argument(
        "--task", "-t", help="Task name (load from task_definitions/<name>.json)"
    )
    agent_runtime_p.add_argument("--model", help="Override model")
    agent_runtime_p.add_argument("--tools", nargs="*", help="Enabled tool names")
    agent_runtime_p.add_argument(
        "--server", action="store_true", help="Start HTTP server"
    )
    ...
    iterate_p.add_argument(
        "topic", nargs="?", default="未命名探索主题", help="要发起探索的主题"
    )
    iterate_p.add_argument(
        "--mock",
        action="store_true",
    ...
    wave2_p.add_argument(
        "--pretty", action="store_true", help="Indent dashboard JSON"
    )
    wave2_p.add_argument(
        "wave2_args",
        nargs=argparse.REMAINDER,
    ...
    bdsk_p.add_argument(
        "bdsk_subcmd", nargs="?", default="debate", help="debate|simulate (default: debate)"
    )
    bdsk_p.add_argument(
        "topic", nargs="?", default="架构决策与技术选型", help="辩论主题或决策方案"
    )

    # ── journey / panorama / project / monitor ────────────────
    sub.add_parser(
        "journey", help="🗺️ Journey State Graph 状态表达校验器"
    )
    sub.add_parser(
        "panorama",
    ...
    proj_p = sub.add_parser(
        "project", help="🔍 17 项目全景 4D 体检与诊断"
    )
    proj_p.add_argument(
        "project_subcmd", nargs="?", default="inspect", help="inspect|list (default: inspect)"
    )
    proj_p.add_argument(
        "project_name", nargs="?", default="", help="指定项目名称"
    )
    ...
    code_sub = code_p.add_subparsers(
        dest="code_command", parser_class=WorkspaceParser
    )
    ...
    code_impact_p = code_workflow_sub.add_parser(
        "impact", help="分析符号的变更影响面"
    )
    code_impact_p.add_argument("--symbol", help="目标符号名称")
    code_workflow_sub.add_parser(
        "onboarding", help="为 AI 构建项目全貌上下文"
    )
    ...
    compute_p.add_argument(
        "compute_command", nargs="?", help="gateway/mesh/swarm"
    )
    compute_p.add_argument(
        "extra", nargs=argparse.REMAINDER, help="传递给 aetherforge 的参数"
    )

    # ── knowledge / memory / kems / c2g ───────────────────────
    knowledge_p = sub.add_parser(
        "knowledge", help="📚 KOS 知识检索 (search/status/stats)"
    )
    ...
    knowledge_search_p.add_argument(
        "--limit", type=int, default=5, help="结果数 (默认 5)"
    )
    ...
    mem_recall = memory_sub.add_parser(
        "recall", help="意图路由召回（neo4j/temporal 支持 --as-of）"
    )
    mem_recall.add_argument("query", nargs="?", help="查询")
    mem_recall.add_argument(
        "--intent", default=None, help="file_note|temporal_fact|entity_relation|…"
    )
    ...
    mem_recall.add_argument(
        "--agent-profile", dest="agent_profile", default=None
    )
    ...
    mem_write = memory_sub.add_parser(
        "write", help="双轨写入 (+ Neo4j FACT 若配置)"
    )
    mem_write.add_argument(
        "--type", dest="mem_type", required=True, help="semantic|episodic|…"
    )
    ...
    mem_cons = memory_sub.add_parser(
        "consolidate", help="sleep-time 巩固 (默认 dry-run)"
    )
    mem_cons.add_argument(
        "--live", action="store_true", help="非 dry-run"
    )
    mem_cons.add_argument(
        "--dry-run", dest="dry_run", action="store_true", default=True
    )
    mem_cons.add_argument(
        "--phases", default=None, help="comma-separated phases"
    )
    ...
    for kref_name in ("knowledge-ref", "kref"):
        mem_kref = memory_sub.add_parser(
            kref_name, help="ADR-0315 引用元数据 (无正文)"
        )
    ...
    kems_p = sub.add_parser(
        "kems", help="🧬 KEMS 域治理 (domains/status/scan)"
    )

...
_DISPATCH = {
    ...
    "search": lambda args: do_default({"action": "list", **args}),
    "ingest": lambda args: do_default({"action": "list", **args}),
    "validate": lambda args: do_default({"action": "health", **args}),
    "register": lambda args: do_default({"action": "list", **args}),
    "list": lambda args: do_default({"action": "list", **args}),
    "run": lambda args: do_default({"action": "status", **args}),
    "get": lambda args: do_default({"action": "health", **args}),
    "sync": lambda args: do_default({"action": "market", **args}),
}
...
if __name__ == "__main__":
    ...
    print("⚠️ Forge 独立 CLI 已弃用，请使用 cockpit 替代", file=sys.stderr)
    sys.exit(serve())

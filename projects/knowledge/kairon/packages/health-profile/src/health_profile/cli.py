"""health-profile stdio CLI shim (BOS transport=stdio).

匹配 agora BOS stdio 适配器协议:
- action 从 argv 尾参取 (如 `python -m health_profile.cli query` 的 `query`)
...
修复: bos://persona/health-profile/{query,update} 声明用 `-m health_profile.cli`,
...
from health_profile.__main__ import _call_action

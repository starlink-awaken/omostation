    ...
    def create_key(
        ...
        if expires_days > 0:
            expires = time.strftime(
                "%Y-%m-%dT%H:%M:%SZ", time.gmtime(time.time() + expires_days * 86400)
            )
        ...
        for row in rows:
            ...
            if ":" in stored:
                ...
                expected = hashlib.pbkdf2_hmac(
                    "sha256", secret.encode(), salt, 6000
                ).hex()
            ...
            if key.expires_at and key.expires_at < time.strftime(

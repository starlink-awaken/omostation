    ...
    try:
        ...
        if args.tenant_cmd == "list":
            ...
            for t in tm.list_tenants():
                svcs = ", ".join(t.get("services", [])) if t.get("services") else "(all)"
                print(f"  {t.get('name', '?'):20s}  rate: {t.get('rate_limit', 0):4d} req/min  services: {svcs}")
    ...
    try:
        ...
        if args.market_cmd == "list":
            ...
            for s in mkt.list_all():
                ...
                print(f"  {'':20s}  repo: {s.get('repo', '?'):30s}  type: {s.get('type', '?')}")
                print()
            ...
            for s in results:
                ...
                print(f"  {'':20s}  repo: {s.get('repo', '?')}  tags: {', '.join(s.get('tags', []))}")
                print()
            ...
            print(f"Published: {result.get('name', '?')} (repo: {result.get('repo', 'N/A')})")
        return 0
    except CLIError as e:
        out.print_error(e.message, e.suggestion)
        return e.exit_code
    ...
    try:
        ...
        if args.key_cmd == "create":
            ...
            for k in keys:
                ...
                print(f"  {k.get('key_id', '?'):20s} {k.get('name', '?'):20s} {status:8s} {', '.join(k.get('scopes', []))}")
            ...
            if result is None:
                out.print_error(f"Key '{args.key_id}' not found", suggestion="使用 'agora key list' 查看所有 Key")
                return 1
...
def cmd_audit(args):
    ...
    try:
        ...
        if args.stats:
            ...
            for e in entries:
                print(f"  [{e.get('timestamp', '?')}] {e.get('actor', '?'):20s} -> {e.get('action', '?'):25s} | {e.get('result', '?')}")
        return 0
    except CLIError as e:
        out.print_error(e.message, e.suggestion)
        return e.exit_code
    ...
    try:
        if args.proto_cmd == "compile":
            ...
            if ret == 0:
                ...
                out.print_success(f"Compiled: {base}_pb2.py, {base}_pb2_grpc.py -> {out_dir}")
            else:
                out.print_error(f"protoc failed with exit code {ret}", suggestion="检查 proto 文件语法")
                return 1
        return 0
    except CLIError as e:
        out.print_error(e.message, e.suggestion)

...
def cmd_accounting_top(args):
    ...
    if args.json:
        ...
        for i, row in enumerate(top, 1):
            rows.append([str(i), row['caller_id'], f"{row['total_cost']:.6f}", str(row['call_count'])])
        out.print_table(["#", "Caller", "Cost (USD)", "Calls"], rows,
...
def cmd_accounting_report(args):
    ...
    if args.json:
        ...
        if report["by_service"]:
            rows = [[svc['service_name'], str(svc['call_count']), f"{svc['total_cost']:.6f}"]
                    for svc in report["by_service"]]
            out.print_table(["Service", "Calls", "Cost (USD)"], rows, title="Breakdown by Service")
    return 0

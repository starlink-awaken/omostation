...
def get_agora_pids():
    ...
    try:
        ...
        result = subprocess.run(["pgrep", "-f", "agora"], capture_output=True, text=True)
...
def main():
    logger.info(f"Agora Watchdog started. Monitoring {AGORA_HEALTH_URL} every {CHECK_INTERVAL}s")
    failures = 0

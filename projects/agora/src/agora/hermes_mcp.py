...
class HermesToolRegistry:
        ...
        try:
            ...
            for r in list(items)[:max_results]:
                results.append({
                    ...
                    "snippet": str(getattr(r, "snippet", r.get("snippet", r.get("content", ""))))[:300],
    ...
    def _mock_search(self, query: str, max_results: int) -> list[dict[str, Any]]:
        q = query.lower()
        base = [
            {"id": "1", "title": "Knowledge Graph Construction Pipeline", "snippet": "End-to-end pipeline for extracting entities and relationships from unstructured text using LLM-based entity resolution.", "score": 0.94, "source": "kairon/kos"},
            {"id": "2", "title": "Multi-Agent Orchestration with Agora", "snippet": "Agora provides MCP-compatible gateway routing, service discovery, and circuit breaking for multi-agent coordination.", "score": 0.91, "source": "kairon/agora"},
            {"id": "3", "title": "Semantic Search over Knowledge Graphs", "snippet": "Hybrid vector + keyword search over ontology-grounded knowledge graphs using embedding models.", "score": 0.87, "source": "kairon/kos"},
            {"id": "4", "title": "Agent Runtime Engine Architecture", "snippet": "Agent runtime provides orchestrator, DSL parser, swarm protocol, governance, and capability discovery.", "score": 0.83, "source": "kairon/agent-runtime"},
            {"id": "5", "title": "Circuit Engine for Fault-Tolerant Workflows", "snippet": "YAML-defined circuit DAGs with step-level retry, timeout, and conditional branching.", "score": 0.79, "source": "kairon/core-models"},
            {"id": "6", "title": "Neural Center Service Registry", "snippet": "NeuralCenter provides service registration, health-aware discovery, and signal routing.", "score": 0.74, "source": "kairon/core-models"},
        ]
        matched = [r for r in base if q in r["title"].lower() or q in r["snippet"].lower()]  # type: ignore[attr-defined]
    ...
    def _mock_agents(self) -> list[dict[str, Any]]:
        ...
        return [
            {"id": "a1", "name": "Researcher", "status": "online", "capabilities": ["search", "analyze", "summarize"], "lastSeen": now},
            {"id": "a2", "name": "Code Assistant", "status": "busy", "capabilities": ["generate", "review", "refactor"], "lastSeen": now - 8000},
            {"id": "a3", "name": "Data Analyst", "status": "online", "capabilities": ["query", "visualize", "report"], "lastSeen": now - 30000},
            {"id": "a4", "name": "Scheduler", "status": "offline", "capabilities": ["plan", "dispatch", "monitor"], "lastSeen": now - 7200000},
        ]
        ...
        try:
            ...
            for t in scheduler._tasks.values():
                tasks.append({
                    ...
                    "agent": t.agent_id,
                    "progress": 1.0 if t.status.value == "completed" else (0.5 if t.status.value == "running" else 0.0),
    ...
    def _mock_tasks(self) -> list[dict[str, Any]]:
        now = int(time.time() * 1000)
        return [
            {"id": "t1", "description": "Analyze knowledge graph connectivity", "status": "running", "agent": "Researcher", "progress": 0.75, "created": now - 600000},
            {"id": "t2", "description": "Generate quarterly summary report", "status": "completed", "agent": "Data Analyst", "progress": 1.0, "created": now - 3600000},
            {"id": "t3", "description": "Index new document corpus", "status": "running", "agent": "Scheduler", "progress": 0.3, "created": now - 120000},
            {"id": "t4", "description": "Validate ontology schema", "status": "pending", "agent": "Researcher", "progress": 0.0, "created": now - 5000},
        ]
    ...
    async def _tool_agent_chat(self, message: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"response": result.get("response", result.get("output", str(result)))}
        except ImportError:
            return {"response": f"Received: '{message}'. Connect to agent-runtime for live interaction."}
        ...
        try:
            ...
            for svc in (registry.list_all() if hasattr(registry, "list_all") else []):
                services.append({
                    ...
                    "lastHeartbeat": int(svc.last_health_check * 1000) if svc.last_health_check else int(time.time() * 1000),
                })
        except (ImportError, AttributeError):
    ...
    def _mock_services(self) -> list[dict[str, Any]]:
        now = int(time.time() * 1000)
        return [
            {"name": "Kairon KOS API", "status": "healthy", "lastHeartbeat": now - 2000},
            {"name": "Agora MCP Gateway", "status": "healthy", "lastHeartbeat": now - 1000},
            {"name": "Agent Runtime", "status": "healthy", "lastHeartbeat": now - 5000},
            {"name": "Knowledge Graph Indexer", "status": "healthy", "lastHeartbeat": now - 15000},
            {"name": "Task Scheduler", "status": "degraded", "lastHeartbeat": now - 60000},
            {"name": "Legacy SharedBrain Bridge", "status": "offline", "lastHeartbeat": now - 300000},
        ]
        ...
        try:
            ...
            for a in manager.get_active() if hasattr(manager, "get_active") else []:
                alerts.append({
                    ...
                    "severity": a.get("severity", "info") if isinstance(a, dict) else getattr(a, "severity", "info"),
                    "message": a.get("message", "") if isinstance(a, dict) else getattr(a, "message", ""),
                    "source": a.get("source", "") if isinstance(a, dict) else getattr(a, "source", ""),
                    "timestamp": a.get("timestamp", int(time.time() * 1000)) if isinstance(a, dict) else int(time.time() * 1000),
                })
        except (ImportError, AttributeError):
    ...
    def _mock_alerts(self) -> list[dict[str, Any]]:
        now = int(time.time() * 1000)
        return [
            {"id": "a1", "severity": "warning", "message": "Task Scheduler response time exceeds 500ms threshold", "source": "Latency Monitor", "timestamp": now - 120000},
            {"id": "a2", "severity": "info", "message": "KOS index cycle completed: 12,847 entities indexed", "source": "Indexer", "timestamp": now - 600000},
            {"id": "a3", "severity": "info", "message": "Researcher agent dispatched to analyze graph connectivity", "source": "Agent Runtime", "timestamp": now - 900000},
        ]
    ...
    def _setup_tools(self) -> None:
        ...
        tools = [
            ...
            ("health.services", self._tool_health_services, "List service health status"),

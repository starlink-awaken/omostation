...
def tool_design(
    ...
    **kwargs,
    ...
    node = {
        "id": f"{m2_type.upper()}-{datetime.now(UTC).strftime('%Y%m%d%H%M%S')}",
        "type": m2_type,
        "name": name,
    ...
    **kwargs,
    ...
    for i in range(count):
        ...
        node = {
            ...
            "stage": schema.lifecycle_stage.value if schema.lifecycle_stage else None,
            "required_properties": {
                prop: prop_def.get("description", "") for prop, prop_def in schema.required_properties.items()
            },
            "optional_properties": {
                prop: prop_def.get("description", "") for prop, prop_def in schema.optional_properties.items()
            },
...
def _derive_transitive(by_type: dict[str, Any], result: dict[str, Any]) -> None:
    """传递推理: A→B, B→C ∴ A→C"""
            ...
            if dep:
                for sub_dep_id in dep.get("properties", {}).get("dependencies", []):
                    if sub_dep_id != model.get("id"):
                        result["transitive_chains"].append(
                            {
                                "from": model.get("id"),
                                "via": dep_id,
                                "to": sub_dep_id,
...
def _derive_risks(by_type: dict[str, Any], result: dict[str, Any], strict_mode: bool = False) -> None:
    """风险推理: 扫描模式，检测风险"""
    models = result.get("models", [])
    for model in models:
...
def _derive_gaps(by_type: dict[str, Any], result: dict[str, Any]) -> None:
    """缺口发现: 检测 M2 类型覆盖"""
    models = result.get("models", [])
...
def tool_derive(
    ...
    **kwargs,
    ...
    """
    models = models or []
    by_type = {m.get("type"): m for m in models}
    result: dict[str, Any] = {
        "insights": [],
        "risks": [],
        "gaps": [],
    ...
    if include_transitive:
        _derive_transitive(by_type, result)

    # 2. 风险推理: 基于推导规则
    if include_risks:
        _derive_risks(by_type, result)

    # 3. 缺口发现: 检测 M2 类型覆盖
    if include_gaps:
        _derive_gaps(by_type, result)
    ...
    return {
        ...
        "total_findings": len(result["risks"]) + len(result["gaps"]) + len(result["transitive_chains"]) + derivation_summary["triggered"],
    }
...
def _validate_required_properties(model: dict[str, Any], result: dict[str, Any], strict_mode: bool) -> None:
    """校验必填属性"""
...
def _validate_rules(model: dict[str, Any], result: dict[str, Any], strict_mode: bool) -> None:
    """校验规则表达式"""
    props = model.get("properties", {})
    schema = model["_schema"]
...
def _validate_state_machine(model: dict[str, Any], result: dict[str, Any]) -> None:
    """校验状态机一致性"""
    status = model.get("status", "")
    schema = model["_schema"]
    ...
    elif result.get("strict") and status in schema.state_machine:
...
def tool_validate(
    ...
    **kwargs,
    ...
    for m in models:
        ...
        # 校验必填属性
        _validate_required_properties(m, result, strict)

        # 校验校验规则
        _validate_rules(m, result, strict)

        # 校验状态机
        _validate_state_machine(m, result)

    passed = len(result["errors"]) == 0
    if strict:
        passed = passed and len(result["warnings"]) == 0
...
def tool_connect(
    ...
    **kwargs,
    ...
    """建立模型间关联 [EXPERIMENTAL: 当前为 Stub 实现，仅生成边字典，未持久化]"""
    edge = {
        "source_id": source_id,
        "target_id": target_id,
    ...
    return {"success": True, "edge": edge, "status": "EXPERIMENTAL"}
...
def _detect_single_drift(model: dict[str, Any], snapshot: dict[str, Any], result: dict[str, Any]) -> None:
    """单模型 vs 单快照的漂移检测"""
    ...
    for key in set(model_props.keys()) | set(snapshot_props.keys()):
...
def _detect_batch_drift(
    models: list[dict[str, Any]], snapshots: list[dict[str, Any]], result: dict[str, Any]
) -> None:
    """批量模型 vs 批量快照的漂移检测"""
...
def tool_evolve(
    ...
    **kwargs,
    ...
    if model and snapshot:
        _detect_single_drift(model, snapshot, result)

    # 批量漂移检测
    if models and snapshots:
        _detect_batch_drift(models, snapshots, result)

    drift_detected = len(result["drifts"]) > 0
    ...
    if drift_detected:
        result["suggestions"].append(
            {
                "action": "update",
                "description": f"检测到 {len(result['drifts'])} 处漂移，建议更新模型",
...
def tool_monitor(
    ...
    **kwargs,
    ...
    health["health_score"] = MAX_HEALTH_SCORE if not health["issues"] else max(0, 100 - len(health["issues"]) * HEALTH_ISSUE_PENALTY)
...
def tool_deploy(
    ...
    **kwargs,
    ...
    """将模型部署到目标环境 [EXPERIMENTAL: 当前为 Stub 实现，无实际部署操作]"""
    if model is None:
        return {"success": False, "error": "缺少模型数据"}
...
def tool_observe(
    ...
    **kwargs,
    ...
    """观测模型运行时行为 [EXPERIMENTAL: 当前为 Stub 实现，返回空 metrics]"""
    observations = []
    ...
    return {"success": True, "status": "EXPERIMENTAL", "observations": observations}
...
def tool_archive(
    ...
    **kwargs,
    ...
    """归档过期模型 [EXPERIMENTAL: 当前为 Stub 实现，仅返回归档字典，未实际归档到目录]"""
    if model is None:
        return {"success": False, "error": "缺少模型数据"}

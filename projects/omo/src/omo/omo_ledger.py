...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--message", type=str, help="Ledger commit message/reason", default="Routine Governance Snapshot")
    ...
    snapshot = {
        ...
        "metrics": {
            "active_tasks": len(list((omo_dir / "tasks" / "active").glob("*.yaml"))) if (omo_dir / "tasks" / "active").exists() else 0,
            "planned_tasks": len(list((omo_dir / "tasks" / "planned").glob("*.yaml"))) if (omo_dir / "tasks" / "planned").exists() else 0,
        },
        "system_state": yaml.safe_load(system_yaml.read_text()) if system_yaml.exists() else None,
        "goals": yaml.safe_load(goals_yaml.read_text()) if goals_yaml.exists() else None,

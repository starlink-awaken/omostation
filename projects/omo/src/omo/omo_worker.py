...
def dispatch_task(
    ...
    if debt_dispatch_file.exists():
        ...
        if debt_state.get("priority") == "P0" and task.get("task_type") != "tech_debt":
            raise ValueError("Task Gate Blocked: Technical debt is P0. You must dispatch a tech_debt task before any new feature tasks.")
    ...
    if depends_on:
        for dep_id in depends_on:
            ...
            if _find_task_file_safe(omo / "tasks" / "planned", dep_id) or _find_task_file_safe(omo / "tasks" / "active", dep_id):

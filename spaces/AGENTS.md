...
- **direct-omo-io**: Scripts must route writes to `.omo/` through `omo CLI`, `projects/omo` core, or the c2g ingress now folded into `projects/omo/` (ADR-0412) — never via raw `open()/mkdir()/write_text()`.

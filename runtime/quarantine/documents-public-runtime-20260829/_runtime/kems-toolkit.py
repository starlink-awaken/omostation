...
规范位置：@公共/_runtime/kems-toolkit.py | v1.0 | 2026-07-31
  · 跨域共享逻辑的唯一实现，域内不复制逻辑，只传参。
  · 各域入口 <域>/_runtime/check-kems-update.py 通过符号链接指向
    @公共/_runtime/check-kems-update.py，由包装器自动定位本文件并透传 --root。
  · 任意域单独使用：python3 kems-toolkit.py --root <域根> [--mode check|health]
"""

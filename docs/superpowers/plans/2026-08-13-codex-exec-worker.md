...
**Architecture:** 根仓 bounded adapter 固定调用 `codex exec --approve-for-me --ephemeral --ignore-user-config --json`。Orca 只追踪 Run/Task/Dispatch，OMO 继续拥有任务与完成真相；adapter 只输出最终消息和脱敏回执。
...
- [ ] **Step 1: Add registry contract test**
...
- [ ] **Step 4: Run focused registry and OMO admission regressions**
...
- [ ] **Step 2: Run supervised Orca task**

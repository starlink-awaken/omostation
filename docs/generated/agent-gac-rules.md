...
<!-- Run: python3 bin/gac/gac-export-agents.py to regenerate. Source: governance-checks.yaml (210 rules) -->
...
| 规则 ID | 层 | 检查类型 | 描述 |
|---------|:--:|---------|------|
| `CR-ADMISSION-01` | L0 | legacy_index | Agora 注册新路由前必须通过 AdmissionPort 准入控制（五大部件义务）。 实现由 I0 SPI / BOS 绑定（realized_by），L0 不点名具体实现类。
 |
| `CR-AGT-ASI-01` | L0 | placeholder | ADR-0370 引用; 真实规则由 PR #931/#934 接管, 此处仅占位以满足 R-GOV-1. |
| `CR-AGT-ASI-10` | L0 | placeholder | ADR-0370 引用; 真实规则由 PR #931/#934 接管, 此处仅占位以满足 R-GOV-1. |
| `CR-C2G-INGRESS-01` | L0 | legacy_index | `projects/c2g` 只能向 `.omo/tasks/planned/` 物化任务，不得直写 active/done |
| `CR-C2G-INGRESS-PRECHECK-01` | L0 | legacy_index | eCOS v6 自愈式 Ingress 约束: 所有引入新端口或新服务名的 Pitch 在
...
| `CR-OMLX-MESH-GATE-01` | L0 | legacy_index | eCOS v6 omlx 算力网格约束: 所有跨节点 LLM completions 必须通过
...
| `CR-P76-7-2-TIER-GRACEFUL-FALLBACK` | meta | schema_integrity | LLM provider 3-tier — aetherforge → ollama → heuristic. 对应 P76-7-2 (tier-graceful-fallback) |
...
| `CR-P76-7-5-RESOLVE-NOT-STUB` | meta | audit_chain | 物理未实现的 placeholder, 必须决议. 对应 P76-7-5 (resolve-not-stub) |
...
| `X2-FRESH-DOC-LIFECYCLE` | meta | legacy_index | 文档 ownership/lifecycle 健康度必须持续维护 |
...
| `CR-CROSS-REPO-REGISTRY-CONSISTENT` | meta | registry_integrity | 保证跨仓库的注册表/元数据一致性 — 被 CR-CROSS-REPO-CONSISTENT (Phase 1 detector, 已 active) 覆盖. |
...
| `CR-KOS-CONSENSUS-RAG-01` | L0 | legacy_index | eCOS v6 Epigenetic Memory 约束: CLAUDE.md 的 Consensus 注入必须
通过 gac-consensus-inject.py 的 RAG 机制（TF-IDF 或 embed-bge），
不允许... |
| `CR-KOS-ONTOLOGY-DRIFT-01` | L0 | legacy_index | eCOS v6 物理算力资产自省约束: 所有在算力网格中活动的物理计算节点 (ComputeNode)，
必须登记在 docs/project-registry.yaml 中并由 gac-kos-sync.py 同步至 KOS SQL... |
...
| `CR-P76-8-3-DEAD-PATH-TOOL-FALLBACK` | meta | consistency_drift | 工具检查应该 first-level + projects/*/first-level 双路 fallback. 对应 P76-8-3 (dead-path-tool-fallback) |
| `CR-P76-8-4-INCREMENTAL-COMMIT` | meta | hygiene_case | 每 sub-task 完成立即 commit, 防 X-Plane clean worktree 丢失工作. 对应 P76-8-4 (incremental-commit-anti-clean) |

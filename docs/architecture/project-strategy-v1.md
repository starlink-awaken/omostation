...
**3 个月目标**: 至少 **3 张场景卡升级到 `assisted`** lifecycle, 每张有 ≥30 samples + calibration ≥0.6.
...
| S3 | activation 评审门槛过高 (calibration ≥0.6) | 缺乏 review 校准数据 | 卡在 shadow 跨年 |

### 1.4 落地路径

- **S1 短期 (2 周)**: 写一份"主人场景卡使用剧本" — 5 步可走完, 每步 < 60 秒
- **S2 中期 (1 个月)**: 引入 `mini-shadow` 模式 — 不要求 30 samples, 只要 3 samples + 主人手勾"有用" 即可升级到 assisted
...
按"能力族"分类, 当前共有 ~24 个能力族, 477 个 bin 工具. 主要能力域:

| 域 | 代表能力 | 成熟度 |
|----|----------|--------|
| 治理 | gac-local-gate, compass_radar, drift-sweep, maturity-scorecard, maturity-align | 8.2/10 |
...
- **F2 入口分散**: 477 个 bin 工具无统一入口, 新用户进入困难
...
| 控制 | 477 个 bin 工具 + 38 个 skill — 选择过载 | 1 个 cockpit 入口 |
...
- **E1 工具过载**: 477 个工具 + 38 个 skill, 没有"该用什么"指南
...
**"能精确测量 139 条 GaC 规则的遵守度, 无法测量系统为主人省下的时间。"** — 这是 v1 阶段的核心错配. v2 阶段已开治本, v3 完成实证.
...
| 层级 | 工作 | 工具 | 成熟度 |
|------|------|------|--------|
| L0 协议 | 139 条 GaC 规则 + 28 条 L0 约束 | bin/gac/gac-validate | 9/10 |
...
- **M1 入口过载**: 477 个工具 + 38 个 skill 缺乏统一引导
...
| GaC 不可绕过 | 139 条规则 + 增量 quota | gac-local-gate, CI blocking |
...
| 约束 | 默认值 | 权衡 |
|------|--------|------|
| 工具 ≤ 477 | T6-05 减法配额 | 新增工具须配对归档/删除 (增量) |
| 规则 ≤ 142 | T6-05 减法配额 | 同上, 现 baseline = 142 |
| ADR ≥ 361 | 数量 baseline | 新增须留档 |
| 场景卡 activation ≥ 0.6 | Y1Q4-T7-01 | 30 samples 校准 |
...
| **shadow 卡片挂年** | 场景卡停在 shadow 跨季度 | mini-shadow 模式 + 周仪式 |
...
|------|-----------|------------|----------|
| 1 场景 | 3/9 assisted | 9/9 assisted, 3/9 autonomous | 全部 autonomous + 自进化 |
...
| R2 | 跨季度 shadow 场景卡 (S1) | 高 | 中 | mini-shadow 模式 (3 samples 升级) |
...
| D2 | 场景激活策略 | 引入 mini-shadow 模式, 3 samples 即可升级 assisted |
| D3 | 入口策略 | cockpit 为唯一入口, 477 个工具被收编 |

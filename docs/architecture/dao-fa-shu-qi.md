...
- CR-SFOP-01：每个 `COMP-WS-*` 必须声明合法 `sfop_slot` 与 `dao_layer`（缺失/非法 = fail-closed）。节点目录未 init 时 skip/warn，不因本地缺子模块锁主干。
- CR-SFOP-02：活跃 Project 中 `S` 槽必须恰好一个且为 `COMP-WS-omo`。

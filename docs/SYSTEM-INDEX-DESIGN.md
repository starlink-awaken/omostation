...
用户/Agent
    ...
    │
    ├─→ .omo/_truth/registry/ ─→ bin/gac/gac-validate.py (CI 门禁)
    │
    └─→ docs/*.md (架构图/路线图/能力图)
```
...
**CI 门禁**: `bin/ssot/doc-ssot-lint.py` 检测硬编码冲突。
...
> - method: 脚本生成 (`python3 bin/ssot/gen-projects-index.py`)
...
> - method: 脚本生成 (`python3 bin/ssot/gen-tools-index.py`)
...
| gac-validate.py | 规则验证 | `python3 bin/gac/gac-validate.py --gate` | bin/ |
| gac-drift.py | 漂移检测 | `python3 bin/gac/gac-drift.py` | bin/ |
| ... | ... | ... | ... |
...
> - method: 脚本生成 (`python3 bin/ssot/gen-knowledge-index.py`)
...
> - method: 脚本生成 (`python3 bin/ssot/gen-agents-index.py`)
...
## 按层分类
<!-- AUTO-GENERATED by bin/ssot/gen-projects-index.py -->
| 层 | 项目 | 栈 | ... |
| L0 | ecos | Python/uv | ... |
<!-- END AUTO-GENERATED -->
```
...
python3 bin/mof/gen-project-registry.py --write
# 3. 重新生成索引
python3 bin/ssot/gen-projects-index.py --write
...
# 3. 重新生成知识索引
python3 bin/ssot/gen-knowledge-index.py --write
...
# 1. 在 bin/ 下创建工具
# 2. 重新生成工具索引
python3 bin/ssot/gen-tools-index.py --write
...
- id: index-drift-check
  ...
  entry: python3 bin/ssot/check-index-drift.py
  language: python
...
| `bin/ssot/gen-projects-index.py` | project-registry.yaml | INDEX-PROJECTS.md | 项目变更时 |
| `bin/ssot/gen-tools-index.py` | bin/ + scripts/ + .agents/ | INDEX-TOOLS.md | 工具变更时 |
| `bin/ssot/gen-knowledge-index.py` | .omo/_knowledge/ | INDEX-KNOWLEDGE.md | 知识变更时 |
| `bin/ssot/gen-agents-index.py` | .claude/ + .codex/ + .agents/ | INDEX-AGENTS.md | Agent 变更时 |
| `bin/ssot/check-index-drift.py` | 各索引 + 各 SSOT | exit code | CI 门禁 |
...
在 `bin/gac/gac-local-gate.py` 的 `CHECKS` 中新增：

```python
# 系统索引漂移检测 (X4 一致性)
("index-drift-check", ["bin/ssot/check-index-drift.py", "--json"]),
```
...
| 脚本 | GaC 角色 | 触发方式 |
|------|---------|---------|
| `bin/ssot/gen-projects-index.py` | executor for `CR-INDEX-SYSTEM-SSOT` | CI 门禁 / 手动 |
| `bin/ssot/gen-tools-index.py` | executor for `CR-INDEX-SYSTEM-SSOT` | CI 门禁 / 手动 |
| `bin/ssot/gen-knowledge-index.py` | executor for `CR-INDEX-SYSTEM-SSOT` | CI 门禁 / 手动 |
| `bin/ssot/gen-agents-index.py` | executor for `CR-INDEX-SYSTEM-SSOT` | CI 门禁 / 手动 |
| `bin/ssot/check-index-drift.py` | executor for `CR-INDEX-GENERATION-FRESH` | CI 门禁 |

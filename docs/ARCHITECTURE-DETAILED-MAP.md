...
| 项目 | 层 | 核心模块 | 职责 |
|------|:--:|---------|------|
| **bus-foundation** | X | `bus/` (Data/Event/Control 三平面) + `metrics/` + `retry/` + `dlq_*` + `schema/` + `observability/` + `bin/bus-dlq` | Omni-Bus 基础设施 v0.3.0: 9 backends + Middleware Pipeline (R89 Metrics → R90 Retry+CB → R91 DLQ Redaction) + Schema Registry (R92) + OTel Trace (R94) + 5-segment cron (R96). 218 tests, 8 new modules, 1 CLI. 零依赖叶子, 被 7 个项目 import |

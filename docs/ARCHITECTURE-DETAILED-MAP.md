...
| 模块组 | 目录 | 职责 | 核心数据结构 |
|--------|------|------|-------------|
| SSOT (MOF) | `ssot/mof/` | M3→M2→M1 元模型层次 | MOF M3 (7 阶段), M2 (48 schema), M1 (1315 实例) |
| SSOT 注册表 | `ssot/registry/` | L0 约束 SSOT | `L0-constraints.yaml` (118 GaC 规则) |
...
| **observability** | X | Docker (Langfuse) | 可观测性: HTTP :3000, OTLP trace 采集 |
...
| **声明式注册** | 加规则 = 加 YAML (不改代码) | GaC (118 规则) |

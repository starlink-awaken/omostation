...
> **状态**：规划中（未执行）— 每项落地必须独立走 agent-workflow（ADR-0203），建议 workflow `project-code-change` 或 `project-doc-change`
> **执行顺序建议**：F-01 → F-02 → F-03 → F-05 → F-04 → F-06 → F-07 → F-08 → F-09 → F-10
...
| # | 项 | 证据 | 验收 |
|---|---|---|---|
| F-11 | 凭据泄漏：Authorization 头不进日志 | `server/tools_auth.py:52` | 日志无凭据 |
| F-12 | RBAC 收紧：非 collab 工具默认 require grant | `auth/authorizer.py:14` | 无 grant 拒绝 |
| F-13 | SSRF 白名单：market/multi_instance_middleware 对外 URL 白名单 | `plugins/market/market.py:302` | 越界 URL 拒绝 |
| F-14 | 盲区测试：tools_auth fail-open 断言、oauth2/identity 单测、凭据泄漏回归 | tests/ | 新增测试通过 |

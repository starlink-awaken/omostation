...
- Producer stop/start: `com.l4.resident.event-ingest` + `com.omostation.agent-tick-daemon` bootout → bootstrap. Restart surfaced a pre-existing never-functional plist invocation (`uv <omo-dir> resident ingest --once`: missing `run` verb + nonexistent `resident` console script). Permanent fix: `lib/resident-event-ingest.sh` wrapper + services.yaml registry entry, plist regenerated by `gen-service-configs.py --write` from the canonical checkout (ADR-0435); post-merge restart exit 0.

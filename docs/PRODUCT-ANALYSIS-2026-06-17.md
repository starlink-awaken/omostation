...
因此，当前最优先的不再是继续扩展功能，而是：
1. **恢复核心服务在线**（runtime / agora SSE / cockpit HTTP）
2. **清理工作区脏状态**（子模块、未提交运行时文件）
3. **让治理状态与运行时状态同步**（P0 消肿、orphan 清零）
4. **修复最直接的用户体验缺口**（search 范围误导、dashboard 离线）

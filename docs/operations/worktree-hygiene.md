...
- **Canonical root remote 解析**（ADR-0293）：`claim`/`submit`/`merge` 现在自动解析指向 `starlink-awaken/omostation` 的 remote，不再硬编码 `origin`。解析顺序：`$OMOSTATION_ROOT_REMOTE` env → `omostation-root` named remote → URL 匹配 → fail closed。参见 `bin/gac/resolve-root-remote.sh`。

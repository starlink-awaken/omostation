...
| `consumption` | 业务消费观测状态；没有显式回执时必须是 `not_observed` |
...
- `consumption.status=not_observed` 是当前预期状态。关闭、验证、证据存在和 PR 合并都不能
  推断用户已经阅读、采用或完成业务回写。
...
当前不新增 `OutcomeConsumed` 运行态事件。消费记录需要先确定真实消费者、回写位置、隐私范围
和责任人，再以独立的反馈契约进入 OMO broker。这样可以避免把“工程完成”误报成“业务有效”，
也避免在没有真实场景时建设第二套结果数据库。

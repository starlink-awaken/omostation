...
- **I15 的这次更正（G5 豁免第三次使用）只动一个文件**：workflow `governance-audit`、bet-less 起步、
  ...
  诚实记一笔过程缺陷：I15 的初稿把重复项写成"台账 30 条 list / 29 条 distinct"，即把
  `gitlink-ancestry` 的 surface 计数当成了全台账计数 —— 提交前复跑 `origin/main` 才发现真值是 35 / 34
  （gitlink surface 才是 30 / 29），于是本节按口径拆成两张数。**"复跑推翻自己上一版的计数"是本文件存在的理由**，
  上面 I3 那条"上一行的台账计数有误"引用块是同一种错误的更早实例，这次在落库前被抓住。

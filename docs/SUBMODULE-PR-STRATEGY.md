...
`bin/sync-submodules-push.sh`(主仓 `.git/hooks/pre-push` 调用):
...
```bash
# 1. 起 worktree
bash bin/gac-worktree.sh claim fix-kairon-bug
...
bash bin/gac-worktree.sh submit fix-kairon-bug

# 7. PR review + merge
bash bin/gac-worktree.sh merge fix-kairon-bug
...
- **ISC-SM-1**: worktree submit 后,`bin/submodule-reachability-gate.py --source head --fetch` PASS(gitlink 可达)
...
- 现有机制: `bin/sync-submodules-push.sh` + `bin/submodule-reachability-gate.py`

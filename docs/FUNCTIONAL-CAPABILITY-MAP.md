...
> 所有数字为快照, 权威读源见 `docs/project-registry.yaml`。

| 维度 | 数量 |
|------|------|
| 功能域 | 8 |
| 能力项 | 32 |
| CLI 命令 (入口+子命令) | 见 `project-registry.yaml` 各项目 cli_subcommands |
| MCP 工具总数 | 见 `project-registry.yaml` 各项目 mcp_tools |
| HTTP 端点 | 见 `protocols/port-registry.yaml` |
| 核心源文件 | 见 `project-registry.yaml` 各项目 src_files |
| M1 元模型节点 | 见 `project-registry.yaml: m1_yaml` |
| GaC 治理规则 | 见 `project-registry.yaml: gac.rules_count` |
...
| 代码理解 | codeanalyze | L2 | `codeanalyze scan` | `codegraph_search` / `ast_search` / `rg_search` / `codegraph_callers` / `codegraph_callees` | AST/调用图/影响半径, MCP tools (见 project-registry.yaml) |
...
| 规则注册 | GaC | meta | `gac-validate` / `gac-drift` / `gac-m1-sync` | `check_gac_rule` | 规则 (见 project-registry.yaml: gac.rules_count)声明式注册 + 6 层 drift + M1 实例同步 |
| BOS URI | agora | I0 | `agora bos` | `resolve_bos_uri` / `mutate_resource` / `list_bos_resources` | 声明式服务路由 (见 project-registry.yaml: bos) |
...
| Lint 静态校验 | omo | L2 | `omo lint` (见 project-registry.yaml) | — | direct-omo-io/schemas/yaml-bypass/god-module/... |

**治理闭环**:
```
规则声明 (gac.rules (见 project-registry.yaml)) → M1 实例 (见 project-registry.yaml: gac.rules_count) → mof-schema-validate (OK)
       ↓                                          ↓
...
| 能力 | 提供者 | 层 | CLI | MCP Tools | 描述 |
|------|--------|:--:|-----|-----------|------|
| MCP Hub | agora | I0 | `agora mcp` | MCP tools (见 project-registry.yaml: agora) | 服务发现/路由/代理, SmartRouter |
| BOS 路由 | agora | I0 | `agora bos` | `resolve_bos_uri` / `read_resource` / `mutate_resource` / `list_bos_resources` / `list_bos_domains` / `get_bos_schema` | 见 project-registry.yaml: bos |
...
| 蜂群协调 | agora | I0 | — | (3 tools) | UDP :port (见 port-registry.yaml) 蜂群发现 |

**通信架构**:
```
Agent → agora MCP :port (见 port-registry.yaml) → resolve_bos_uri → 后端服务
                         ↓
                    BOS 中间件 (限流/熔断/缓存)
                         ↓
...
| 能力 | 提供者 | 层 | CLI | MCP Tools | 描述 |
|------|--------|:--:|-----|-----------|------|
| MOF 元模型 | ecos | L0 | `cockpit mof` | `ssot_check` / `ssot_derive` / `ssot_compile` / `ssot_evolve` / `ssot_stats` / `ssot_sync` / `ssot_extract` | M3→M2→M1 三层元模型, M1 节点 (见 project-registry.yaml: m1_yaml) |
| MOF 工具集 | ecos | L0 | (见 project-registry.yaml: ecos.mof_tools) | — | validate/audit/derive/bridge-sync/state-bridge/enforce/reason/extract/scan/model/schema-validate/... |
...
| 推导引擎 | model-driven | M0 | — | `model-execute` / `model-tools` / `ssot-drift-check` / `cross-stage-check` / `value-roi` | DR 规则 + 工具执行总线 |
...
M2 (schema)   → m2/*.yaml: 48 个类型 schema (含 GacRule)
       ↓ mof-schema-validate
M1 (实例)     → m1/**/*.yaml: 节点 (见 project-registry.yaml: m1_yaml) (含 GaC 规则 (见 project-registry.yaml: gac.rules_count) + 1 元模型 + 96 OMOTask + 25 GOV-* + ...)
       ↓ gac-m1-sync
SSOT          → governance-checks.yaml::gac.rules (规则 (见 project-registry.yaml: gac.rules_count))
```
...
| 能力 | 提供者 | 层 | CLI | MCP Tools | 描述 |
|------|--------|:--:|-----|-----------|------|
| 统一入口 | cockpit | L3 | `cockpit` (见 project-registry.yaml: cockpit) | MCP tools (见 project-registry.yaml) | CLI + Web (:port (见 port-registry.yaml)) + MCP, 唯一人类入口 |
| 研究管线 | cockpit | L3 | `cockpit research` (见 project-registry.yaml) | `research_list` / `research_search` / `research_create` / `research_ask` / `research_archive` / ... | 深度研究管理 (ask/audit/digest/dossier/publish/...) |
| Web Dashboard | cockpit | L3 | `cockpit dashboard` | — | FastAPI :port (见 port-registry.yaml), API router 模块 |
| C2G 编排流 | cockpit | L3 | `cockpit iterate` | — | C2G 双擎迭代流 (MetaOS → Model-Driven → OMO) |
| 域管理 | l4-kernel | L4 | `l4-kernel` | MCP tools (见 project-registry.yaml: l4-kernel) | 域注册 (见 project-registry.yaml: l4-kernel.domains) + KEMS 六面 + 生命周期 + 信号 |
...
**入口架构**:
```
人类用户 → cockpit CLI (见 project-registry.yaml: cockpit) / Web (:port (见 port-registry.yaml)) / cockpit-ui (多视图)
AI Agent → agora MCP :port (见 port-registry.yaml) → resolve_bos_uri → 后端
Web/API  → cockpit HTTP :port (见 port-registry.yaml) → /api/* (services/compute/health/events/knowledge/...)
```
...
> BOS 声明式注册表 SSOT: `projects/agora/etc/bos-services.yaml` (见 project-registry.yaml: bos)

...
class TestDispatch:
    ...
    def test_websocket_protocol_dispatches(self):
        inst = {
            "protocol": "websocket",
            "mcp_endpoint": "ws://192.0.2.1:8080",
    ...
    async def test_dispatch_stdio(self):
        result = await dispatch(
            {"protocol": "stdio", "mcp_endpoint": "stdio://test"}, "test.tool", {}
        )
    ...
    async def test_dispatch_mcp_ssrf_blocked(self):
        ...
        result = await dispatch(
            {"protocol": "mcp", "mcp_endpoint": "http://10.0.0.1/mcp"}, "test.tool", {}
        )
        assert result["status"] == "error"
    ...
    async def test_dispatch_rest_ssrf_blocked(self):
        ...
        result = await dispatch(
            {
                "protocol": "rest",
                "mcp_endpoint": "http://10.0.0.1/api",
                "protocol_config": {"method": "GET"},
            },
            "test.list",
            {},
    ...
    async def test_dispatch_ws_invalid_url(self):
        ...
        result = await dispatch(
            {
                "protocol": "websocket",
                "mcp_endpoint": "http://192.0.2.1:8080",
                "protocol_config": {"timeout": 1},
            },
            "test.push",
            {},

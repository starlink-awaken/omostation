...
def test_build_promotion_approval_analytics_packet_classifies_action_queues_and_histograms(
    tmp_path: Path,
):
    _write_yaml(
        tmp_path / ".omo" / "workers" / "promotion" / "approvals" / "current.yaml",
...
def test_build_promotion_approval_analytics_packet_assigns_age_buckets_for_open_requests(
    tmp_path: Path,
):
    _write_yaml(
        tmp_path / ".omo" / "workers" / "promotion" / "approvals" / "current.yaml",
    ...
    assert [item["task_id"] for item in packet["yaml"]["tasks"]] == [
        "TASK-C",
        "TASK-B",
        "TASK-A",
    ]
...
def test_build_promotion_approval_analytics_packet_requires_all_canonical_inputs(
    tmp_path: Path,
):

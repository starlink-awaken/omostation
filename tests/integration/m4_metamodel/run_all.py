...
def test_p1s0_schemas_loaded(verbose: bool = False) -> tuple[bool, str]:
    ...
    return True, "50 M2 schema (含 8 之前未加载) 全部存在"
...
def test_p1s2_migrate_77all(verbose: bool = False) -> tuple[bool, str]:
    ...
    rc, out, err = run(
        [
            ...
            "pyyaml",
            "python",
            "bin/_archive/l0-constraints-migrate.py",
            "--dry-run",
        ],
        timeout=30,
    )
...
def test_p1s2_v2_yaml_exists(verbose: bool = False) -> tuple[bool, str]:
    ...
    rc, _, _ = run(
        [
            ...
            "check-ignore",
            "-q",
            ".omo/_derived/l0-constraints.v2.yaml",
        ]
    )
    return (
        rc == 0,
        f"v2 派生面 {v2.stat().st_size} bytes, submodule gitignored={rc == 0}",
    )
...
def test_p3_docs_generated_gitignored(verbose: bool = False) -> tuple[bool, str]:
    ...
    return (
        rc == 0,
        "gitignored=YES for new derived file" if rc == 0 else f"NOT (rc={rc}, out={out!r})",
    )
...
def test_adr_index_5_entries(verbose: bool = False) -> tuple[bool, str]:
    ...
    return True, "5 条 M4 ADR (0132..0136)"
...
def test_r4b_decisions_quick_ref_exists(verbose: bool = False) -> tuple[bool, str]:
    ...
    return (
        True,
        f"M4 速查表存在, ADR 覆盖 {adr_count}/11, Round {round_present}/{len(rounds)}",
    )
...
def test_r4d_m4_cron_log_gitignored(verbose: bool = False) -> tuple[bool, str]:
    ...
    return (rc == 0, "gitignored=YES" if rc == 0 else f"NOT gitignored (rc={rc})")

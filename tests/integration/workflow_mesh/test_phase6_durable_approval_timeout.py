...
2. After the 7-day policy elapses (injectable ``now``), a scan persists
   ``ApprovalExpired`` and the run is projected to ``unavailable``.
...
from omo.approval_timeout import scan_approval_timeouts
...
def _waiting_approval_run(tmp_path, run_id: str = "it-approval") -> WorkflowMeshStore:
    ...
    store.append(
        new_workflow_event(
            ...
            run_id,
            producer="runtime",
            payload={"approval_id": "workflow", "timeout_after": "P7D"},
        )
    )
...
def test_waiting_approval_survives_restart_with_deadline(tmp_path):
    ...
    assert approval["timeout_after"] == "P7D"
...
def test_seven_day_policy_expires_and_is_durable(tmp_path):
    ...
    assert result["expired"][0]["event_type"] == "ApprovalExpired"

...
def test_fusion_optimization_blueprint_covers_strategy_tactics_and_execution():
    ...
    assert (
        "_control" in text
        and "_truth" in text
        and "_knowledge" in text
        and "_delivery" in text
    )
...
def test_index_links_blueprint_and_meta_retrospective():
    ...
    assert (
        "[omo-fusion-optimization-blueprint.md](plans/omo-fusion-optimization-blueprint.md)"
        in index_text
    )
    assert (
        "[omo-mechanism-and-phase1-3-retrospective.md](../../summaries/omo-mechanism-and-phase1-3-retrospective.md)"
        in process_text
    )
    assert (
        "[omo-fusion-optimization-blueprint.md](../../plans/omo-fusion-optimization-blueprint.md)"
...
def test_convergence_audit_is_indexed_and_live_indexes_do_not_copy_stale_counts():
    ...
    assert (
        "[omo-convergence-audit-2026-05-31.md](_knowledge/management/omo-convergence-audit-2026-05-31.md)"
        in index_text
    )
    assert (
        "[omo-convergence-audit-2026-05-31.md](omo-convergence-audit-2026-05-31.md)"
        in management_text
    )

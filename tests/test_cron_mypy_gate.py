...
class TestCrontabWiring:
    ...
    def test_crontab_has_mypy_baseline_gate_entry(self):
        ...
        assert "mypy-baseline-gate" in text, f"crontab 缺 mypy-baseline-gate 调度行:\n{text}"

    def test_crontab_has_feedback_loop_guard_entry(self):
        text = CRONTAB.read_text(encoding="utf-8")
        assert "feedback-loop-guard" in text, "crontab 缺 feedback-loop-guard 调度行"
...
class TestMypyBaselineGateRuntime:
    def test_gate_dry_run_in_workspace_exits_zero_or_one(self):
        ...
        assert result.returncode in (0, 1), (
            f"mypy-baseline-gate 解析崩溃 (rc={result.returncode}): stderr: {result.stderr[:500]}"
        )
    ...
    def test_gate_dry_run_never_emits_event(self):
        ...
        before = log.read_text().count("mypy_regression_detected") if log.exists() else 0
        subprocess.run(
            [sys.executable, str(MYPY_GATE), "--dry-run"],
            cwd=ROOT,
            capture_output=True,
        ...
        after = log.read_text().count("mypy_regression_detected") if log.exists() else 0
    ...
    def test_gate_resolves_kairon_baseline(self):
        ...
        assert baseline.exists(), (
            f"kairon mypy-baseline.yaml 不存在: {baseline}. mypy-baseline-gate 无 baseline 可比, gate 失效."
        )
    ...
    def test_gate_source_injects_mypypath_src(self):
        ...
        assert "MYPYPATH" in source, "mypy-baseline-gate 必须显式注入 MYPYPATH=src (L0:CR-ENG-MYPY-TRUTH-01 防假绿)"
...
class TestCronExecutionSimulation:
    ...
    def test_feedback_loop_guard_runs_in_workspace(self):
        result = subprocess.run(
            [
                sys.executable,
                str(ROOT / "bin" / "gac" / "feedback-loop-guard.py"),
                "--check",
            ],
            cwd=ROOT,
            capture_output=True,

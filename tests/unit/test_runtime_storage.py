...
async def test_schema_migrates_once_and_enforces_private_wal_pragmas(tmp_path: Path) -> None:
    ...
    assert await reopened.schema_version() == 4
...
@pytest.mark.asyncio
async def test_v3_schema_migrates_to_benchmark_runs(tmp_path: Path) -> None:
    from omlxc.storage import database as storage_database
    ...
    connection.executescript(
        f"""
        {storage_database._V3_SCHEMA_SQL};
    ...
    store = await storage.SQLiteRuntimeStore.open(database)
    assert await store.schema_version() == 4
    assert await store.list_benchmark_runs() == []
    await store.close()

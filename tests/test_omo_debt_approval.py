...
def test_dispatch_entry_requires_approval_only_for_gate_revalidate_items() -> None:
    ...
    assert (
        dispatch_entry_requires_approval(
            find_dispatch_entry(packet, "SB_DECOMPOSITION")
        )
        is True
    )
    assert (
        dispatch_entry_requires_approval(
            find_dispatch_entry(packet, "SB_UNTESTED_PKGS")
        )
        is False
    )
    assert (
        dispatch_entry_requires_approval(find_dispatch_entry(packet, "D2_CI_E2E"))
        is False
    )
    assert (
        dispatch_entry_requires_approval(find_dispatch_entry(packet, "MISSING"))
        is False
    )


def test_build_approval_record_and_paths_use_immutable_run_ref() -> None:
    current_only_path = approval_current_path(
        Path("/tmp/example/.omo"), item_id="SB_DECOMPOSITION"
    )
    ...
    assert current_only_path == Path(
        "/tmp/example/.omo/debt/approvals/SB_DECOMPOSITION/current.yaml"
    )
    assert current_path == current_only_path
    assert record_path == Path(
        "/tmp/example/.omo/debt/approvals/SB_DECOMPOSITION/records/2026-06-10T01-00-00Z.yaml"
    )
    assert record == {
        "item_id": "SB_DECOMPOSITION",
        "approved_by": "omo-governance",

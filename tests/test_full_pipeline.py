...
def test_signal_router() -> None:
    ...
    print("\n=== 信号路由 ===")
    result = run_cmd([
        sys.executable, str(REPO / "bin" / "bc-os" / "signal_router.py"),
        "--calendar", str(REPO / ".omo" / "_delivery" / "personal-signals" / "calendars" / "events.ics"),
        "--json",
    ])
    check("signal_router executes", result.returncode == 0)
    if result.returncode == 0:
        data = json.loads(result.stdout)
        check("calendar routed > 0", data["summary"]["total_routed"] > 0,
              f"routed={data['summary']['total_routed']}")
...
def test_probes() -> None:
    ...
    print("\n=== 探测器心跳 ===")
    result = run_cmd([
        sys.executable, str(REPO / "bin" / "gac" / "probe-heartbeat-monitor.py"),
        "--status",
    ])
...
def test_orchestrator() -> None:
    ...
    print("\n=== 编排引擎 ===")
    result = run_cmd([
        sys.executable, str(REPO / "bin" / "gac" / "unified-orchestrator.py"),
        "--run-all",
    ])

...
def test_ensure_entity_type_column(tmp_path) -> None:
    ...
    conn.execute("CREATE TABLE kos_entities (entity_id TEXT, label TEXT, source_file TEXT)")
    conn.execute("INSERT INTO kos_entities VALUES ('consensus-1', 'Consensus gene', 'x.md')")
    conn.commit()
...
def test_fallback_without_entity_type(tmp_path) -> None:
    ...
    conn = sqlite3.connect(str(db))
    conn.execute("CREATE TABLE kos_entities (entity_id TEXT, label TEXT, source_file TEXT)")
    conn.execute("INSERT INTO kos_entities VALUES ('other-1', 'normal', 'y.md')")
    conn.execute("INSERT INTO kos_entities VALUES ('p74-consensus', 'Consensus pattern', 'z.md')")
    conn.commit()

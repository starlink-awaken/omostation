...
_spec = importlib.util.spec_from_file_location("doc_claims", _BIN / "check-doc-claims.py")
...
def test_interface_with_pointer_clean():
    ...
    findings = doc.check_interface_tests_has_pointer("tests: 260  # as_of 2026-07-26, 见 pytest tests/\n")
    assert findings == []


def test_bare_pass_claim_detected():
    """注入 '100% 通过' → 检出."""
    findings = doc.check_no_bare_pass_claim("# 测试 (100% 通过, 188 tests)", "AGENTS.md")
    assert len(findings) == 1
...
def test_pointerized_md_clean():
    """指针化表述 (无裸数字) → 无漂移."""
    findings = doc.check_no_bare_pass_claim("# 测试 (计数见 pytest tests/ 实际输出 / CI, 不复制数字)", "CLAUDE.md")
    assert findings == []
...
def test_detect_metaos_no_findings():
    ...
    assert result["total_findings"] == 0, f"metaos 文档仍有裸宣称: {result['findings']}"

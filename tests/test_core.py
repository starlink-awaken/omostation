...
from runtime.cron_service.executor import ExecutionResult, _get_interpreter, _kill_process, _read_shebang, execute
...
class TestStartScript:
    ...
    def test_start_script_executes_cron_service_server(self):
        script = (Path(__file__).resolve().parent.parent / 'scripts' / 'start-cron-service.sh').read_text(encoding='utf-8')
...
class TestJobCreate:
    ...
    def test_custom_values(self):
        jc = JobCreate(name='custom', schedule='0 2 * * *', script='/path/to/script.sh', no_agent=False, deliver='origin', timeout=300, enabled=False)
...
class TestDeliverOrigin:
    ...
    def test_successful_weixin_send(self, tmp_path):
        ...
        with patch('runtime.cron_service.delivery.OUTPUT_ROOT', tmp_path):
            with patch.dict(os.environ, {'WEIXIN_ACCOUNT_ID': 'acc1', 'WEIXIN_TOKEN': 'tok1', 'WEIXIN_HOME_CHANNEL': 'chan1'}, clear=True):

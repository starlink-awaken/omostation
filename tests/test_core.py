...
class TestResolveScript:
    ...
    def test_relative_path_in_hermes_scripts(self):
        ...
        # Create a temp file in ~/.hermes/scripts/
...
class TestStartScript:
    ...
    def test_start_script_executes_cron_service_server(self):
        ...
        assert "python3\" -m runtime.cron_service.server" in script or "python3' -m runtime.cron_service.server" in script or "-m runtime.cron_service.server" in script
...
class TestGet:
    ...
    def test_env_var_empty_uses_config(self):
        ...
        # Ensure _cfg is populated with test data
...
class TestJobCreate:
    ...
    def test_custom_values(self):
        jc = JobCreate(name="custom", schedule="0 2 * * *", script="/path/to/script.sh",
                       no_agent=False, deliver="origin", timeout=300, enabled=False)
...
from runtime.cron_service.db import create_job, delete_job, get_job, init_db, list_jobs, record_run, update_job
...
class TestDbCRUD:
    ...
    def test_init_db_creates_tables(self, tmp_path):
        init_db()  # uses test default path
...
class TestRegisterTools:
    ...
    def test_registers_9_tools(self):
        ...
        # tool() should be called 9 times (one per tool)
    ...
    def test_tools_return_json(self):
        ...
        # Collect the actual tool functions

        def capture_tool(**kwargs):
            def decorator(fn):
                # The decorator is called immediately, but fn is returned
                # We need to capture the *inner* wrapper function
                # Actually, @fastmcp.tool() with no args returns a decorator
                return fn
            return decorator

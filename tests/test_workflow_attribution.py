...
def registry(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict:
    ...
    profiles = {
        "test-agent": {"id": "test-agent", "actor": "tester", "allowed_workflows": ["*"]},
        "coordinator": {"id": "coordinator", "actor": "human", "allowed_workflows": ["*"]},
        "planner": {"id": "planner", "actor": "agent-a", "allowed_workflows": ["*"]},
        "executor": {"id": "executor", "actor": "agent-b", "allowed_workflows": ["*"]},
        "orchestrator": {"id": "orchestrator", "actor": "alice", "allowed_workflows": ["*"]},
...
def _context(actor: str = "tester", profile: str = "test-agent") -> dict[str, str]:
    return {"actor": actor, "profile": profile, "project": "", "format": "openspec", "source_file": "", "run_id": ""}
...
def test_start_run_with_parent(registry: dict) -> None:
    record = start_run(
        registry, _workflow(registry), _context(), "child objective", True, False,
...
def test_start_run_ledger_includes_parent(registry: dict) -> None:
    start_run(
        registry, _workflow(registry), _context(), "child", False, False,
...
def test_spawn_run_links_parent(registry: dict) -> None:
    parent = start_run(registry, _workflow(registry), _context("alice", "test-agent"), "parent work", False, False)
    child = spawn_run(
        registry, parent["run_id"], _workflow(registry),
        _context("bob", "test-agent"), "child work",
    )
...
def test_trace_attribution_single_run(registry: dict) -> None:
    root = start_run(registry, _workflow(registry), _context("alice", "test-agent"), "root work", False, False)
...
def test_trace_attribution_chain(registry: dict) -> None:
    root = start_run(registry, _workflow(registry), _context("alice", "orchestrator"), "root", False, False)
    child = spawn_run(
        registry, root["run_id"], _workflow(registry),
        _context("bob", "worker"), "child task",
    )
...
def test_trace_attribution_three_levels(registry: dict) -> None:
    root = start_run(registry, _workflow(registry), _context("human", "coordinator"), "intent", False, False)
    mid = spawn_run(registry, root["run_id"], _workflow(registry), _context("agent-a", "planner"), "plan", False, False)
    leaf = spawn_run(registry, mid["run_id"], _workflow(registry), _context("agent-b", "executor"), "execute", False, False)

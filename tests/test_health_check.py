...
class TestRunCheck:
    ...
    def test_success(self, mock_run):
        ...
        check = {"id": "test", "name": "Test", "dim": "X1", "cmd": ["python3", "/fake/test.py"], "requires": []}
    ...
    def test_missing_dependency(self):
        check = {"id": "test", "name": "Test", "dim": "X1", "cmd": ["python3", "/fake/test.py"], "requires": ["/nonexistent/path"]}
        result = run_check(check)
        assert result["pass"] is None
        assert result["status"] == "skipped"

    def test_missing_script(self):
        check = {"id": "test", "name": "Test", "dim": "X1", "cmd": ["python3", "/nonexistent/script.py"], "requires": []}
        result = run_check(check)
        assert result["pass"] is None
...
class TestFormatReport:
    ...
    def test_format_with_results(self):
        results = [
            {"id": "c1", "name": "Check 1", "dim": "X1", "pass": True, "status": "pass", "reason": "", "duration_ms": 10},
            {"id": "c2", "name": "Check 2", "dim": "X2", "pass": False, "status": "fail", "reason": "error msg", "duration_ms": 5},
        ]

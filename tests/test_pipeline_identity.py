...
class _FakeRouter:
    ...
    async def route(
        self, tool_name: str, args: dict, caller_id="unknown"
    ) -> dict[str, object]:
        self.calls.append(
            {"tool_name": tool_name, "args": args, "caller_id": caller_id}
        )
...
def test_run_passes_structured_identity_to_router() -> None:
    ...
    asyncio.run(
        pipeline.run("identity-aware", {"goal": "ship it"}, caller_identity=identity)
    )
...
def test_pipeline_events_include_normalized_identity_payload() -> None:
    ...
    started = next(
        ...
        and event["payload"]["pipeline"] == "identity-aware"
    )
    completed = next(
        event
        for event in events

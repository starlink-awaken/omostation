...
| `gac-worktree.sh claim` 静默失败 (无输出) | 并发 agent (filter-repo) 把主仓 `core.bare=true`, rev-parse 返空 set -e 静默 exit | `git config core.bare false` (一行可逆); harness 层 `bin/git-health-hook.py` (PreToolUse advisory) 自动检测 (memory `core-bare-anomaly`) |
...
7. close: 写 docs/M4-DECISIONS-INDEX.md (新 ADR 加入),
  ...
  G-Health:  bin/m4-health-score.py --compare  delta ≥ 0
```

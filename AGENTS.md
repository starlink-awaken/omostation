...
7. **B.D.S.K. Virtual Board & Compute Deliberation** — For high-risk architectural changes, trade-offs, or local LLM/edge compute integration, refer to [`.agents/skills/bdsk-virtual-board/SKILL.md`](.agents/skills/bdsk-virtual-board/SKILL.md) to invoke the 4-Corner (`@Builder`/`@Devil`/`@Sage`/`@Keeper`) consensus engine (Mode-A/B). All local/edge LLM inference MUST route through **AetherForge + omlxc v3.2.0 (`bos://compute/aetherforge/infer`)** with dual-mode session/prefix KV cache affinity, in-flight concurrency load-shedding, 3-state circuit breaking + last-resort self-healing probe, and SQLite benchmark telemetry (`make omlxc-benchmark`, `omlxc routes plan <model> --explain`, `omlxc daemon reload`).

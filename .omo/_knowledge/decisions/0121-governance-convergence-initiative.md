...
4. `bin/gac/governance-convergence-lint.py` 0 ERROR (SSOT Convergence, 覆盖 4 维度)
...
- id: CR-X1-FRESHNESS-SEMANTIC
  ...
  executor: bin/gac/governance-convergence-lint.py --rule freshness-semantic
...
- id: CR-L0-MATRIX-PORT-CONSISTENCY
  ...
  executor: bin/ssot/matrix-consistency-lint.py
...
- id: CR-L0-SSOT-PATH-NORM
  ...
  executor: bin/gac/governance-convergence-lint.py --rule ssot-path-norm
...
- id: CR-META-CI-SKIP-MATRIX
  ...
  executor: bin/gac/governance-convergence-lint.py --rule ci-skip-matrix
```

#### 1.3 规则注册闭环 lint

**文件**: `bin/gac/governance-convergence-lint.py` (新建, GCSI 核心工具)
...
**文件**: `bin/gac/governance-convergence-lint.py`
...
subprocess.run(["python3", "bin/gac/evidence-smoke.py", "--quiet"], ...)
...
**文件**: `bin/gac/governance-convergence-lint.py` (新建, 统一收敛 lint)
...
CHECKS = (
    ...
    ("governance-convergence", ["bin/gac/governance-convergence-lint.py"]),
)
```
...
| V7 | evidence-smoke | `python3 bin/gac/evidence-smoke.py` | 回路存活: True |

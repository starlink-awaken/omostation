...
| 交付 | 状态 | 关键产物 |
|------|:---:|---------|
| **radar_cron 真正集成** | ✅ | `bin/knowledge-foundry-cron.py` 9-cron 编排 |
...
### 1.2 WHAT — `bin/knowledge-foundry-cron.py` (radar_cron)

实际跑 9 个 cron deck:

```bash
# 调度点 (systemd timer / cron / k8s cronjob 任选其一):
bin/knowledge-foundry-cron.py          # run all 9 decks
bin/knowledge-foundry-cron.py --dry-run # 仿真
```
...
- id: CR-FOUNDRY-MONITOR
  ...
  target: bin/knowledge-foundry-cron.py
  source_ref: bin/knowledge-foundry-cron.py::main
...
- [x] `bin/knowledge-foundry-cron.py` 创建, --dry-run 跑通

...
| **root repo 迁移** | ✅ | `bin/start-gateway.sh` (9290→LLM_GATEWAY_PORT), `bin/gac-mesh-router.py` (7437→OMLX_MESH_ROUTER_PORT) |
...
修真修真反模式:
- `bin/start-gateway.sh`: `--port 9290` 字面量 → 部署不能无修改切换
- `bin/gac-mesh-router.py`: `PORT = 7437` 字面量 → 不能多实例配不同端口
...
| 文件 | 旧 | 新 | Env Var |
|------|----|----|---------|
| `bin/start-gateway.sh:12` | `--port 9290` | `--port "${LLM_GATEWAY_PORT:-9290}"` | `LLM_GATEWAY_PORT` |
| `bin/gac-mesh-router.py:17` | `PORT = 7437` | `PORT = int(os.environ.get("OMLX_MESH_ROUTER_PORT", "7437"))` | `OMLX_MESH_ROUTER_PORT` |

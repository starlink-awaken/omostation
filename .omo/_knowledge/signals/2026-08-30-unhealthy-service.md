---
id: signal://evolution/2026-08-30-unhealthy-service
status: draft
lifecycle: signal
owner: evolution-agent
last-reviewed: 2026-08-31
source: evolution-proposal
type: ephemeral
status: archived
---

# Signal: Address: unhealthy_service

---
id: ADR-0401
status: draft
lifecycle: signal
owner: evolution-agent
last-reviewed: 2026-08-30
---

- date: 2026-08-30
- owner: evolution-agent
- source: evolution-proposal (2026-08-30)
- risk_level: S2

---

## Context

Auto-generated from evolution proposal. Source: `problem_detector`.

Total proposals in batch: 4

## Proposal Details

### [S2] problem_detector
**Address: unhealthy_service**
- Action: `create_debt_task`
- Severity: high

### [S2] problem_detector
**Address: unhealthy_service**
- Action: `create_debt_task`
- Severity: high

### [S3] external_rss
**[hacker_news] European Commission Revives Push for Encryption Backdoors in ProtectEU Strategy**
- Action: `research_task`
- Severity: low

### [S2] external_rss
**[hacker_news] Europe's summer drought is so extreme that desertification is a growing threat**
- Action: `research_task`
- Severity: low

## Triage

_Pending human review. Auto-generated draft — do not merge without review._

## Follow-ups

- [ ] Human review
- [ ] Promote to a newly numbered ADR only after a real decision is accepted

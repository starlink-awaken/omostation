...
| [omo state sync](uv run python -m omo.cli state sync) | 刷新 `.omo/state/system.yaml` 的统一入口 | omo CLI |
| [scripts/phase3_acceptance.py](../../../scripts/phase3_acceptance.py) | 运行 Phase 3 acceptance baseline | repo `scripts/` |
| [omo worker](uv run python -m omo.cli worker) | `omo worker dispatch/status/reclaim` 运维说明 | omo CLI |

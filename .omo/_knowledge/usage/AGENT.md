...
>
> **Promotion history**：如需查看最近有哪些 planned packet 被正式晋升，不要直接 glob `workers/runs/*-promotion-*.yaml`；先运行 `uv run python -m omo.cli worker task promotion-history --omo-dir .omo`，再以 `.omo/workers/promotion/current.yaml` 作为 canonical read surface。
>
> **Promotion readiness**：如需查看当前 planned queue 里哪些 packet 可以晋升、哪些仍被 gate 挡住，先运行 `uv run python -m omo.cli worker task promotion-readiness --omo-dir .omo`，再以 `.omo/workers/promotion/readiness.yaml` 作为 canonical read surface。
>
> **Promotion approval**：`future-active-l2l3-pending-approval-*.md` 这类共享 backlog-presence note 只说明未来 backlog presence，does not authorize promotion；对 `human_approval_required: true` 的 planned packet，`approval_ref` 必须指向 task-specific promotion approval YAML。需要发起请求时，先运行 `uv run python -m omo.cli worker task promotion-request-approval <TASK_ID> --requested-by <ACTOR> --now <ISO8601> --omo-dir .omo`；需要查看 closure lifecycle 时，运行 `uv run python -m omo.cli worker task promotion-approval-status --omo-dir .omo [--task-id <TASK_ID>] [--now <ISO8601>]`；需要查看 approval artifacts 的 canonical history/index 时，运行 `uv run python -m omo.cli worker task promotion-approval-history --omo-dir .omo [--now <ISO8601>]`；真正 mutation 继续走 `uv run python -m omo.cli governance approve <PROPOSAL_ID> --approver <ACTOR> --now <ISO8601>` + `uv run python -m omo.cli governance apply <PROPOSAL_ID> --now <ISO8601>`；只要 approval record 仍是 `requested`，readiness 继续保持 `approval_invalid`。
>
> **Promotion approval analytics**：如需看当前 approval queue 的 action rollup、blocker histogram 与 request aging，运行 `uv run python -m omo.cli worker task promotion-approval-analytics --omo-dir .omo [--now <ISO8601>]`；该命令读取 approval current/history/readiness 三个 canonical surfaces，并输出 `.omo/workers/promotion/approvals/analytics/current.yaml`。
>
> **Governance overlay**：如需看未来 roadmap / milestone / planned queue / debt watchlist 的统一治理入口，不要改 `current_phase`；运行 `uv run python -m omo.cli worker task governance-overlay-status --omo-dir .omo [--now <ISO8601>]`，并以 `.omo/workers/governance-overlay/current.yaml` 作为 canonical read surface。如需让 OMO 自动推进 overlay 的下一个 planned-task roadmap item，运行 `uv run python -m omo.cli worker task governance-overlay-run-next --omo-dir .omo --actor <ACTOR> [--now <ISO8601>]`；第一版只支持 `.omo/tasks/planned/*.yaml` target refs，debt bundle 仍保持 read-only。治理 overlay 现在有 current active roadmap item continuation 语义：`governance-overlay-run-next` 会先处理当前 `in_progress` item，再考虑新的 pending candidate；不要把 `next_action: idle` 误读为“没有工作”，除非 active item 与 pending item 都不存在。当前 operator 语义里，dispatch or verify the current active roadmap item 仍是一等动作，同时 active roadmap item 的一等动作已扩展为 contract-aware control loop：`dispatch:<TASK_ID>` 表示 safe preclaim/dispatch，`contract:<TASK_ID>` 表示 task packet 尚未声明足够的 deliverables/write scope，`launch:<TASK_ID>` 表示现有 dispatch packet 已具备安全自动启动条件，`verify:<TASK_ID>` 表示 review-ready closeout。autonomous launch requires explicit task deliverables；如果 task packet 的 `deliverables` 为空或无法导出明确 write scope，coordinator 必须 fail closed，而不是猜测写权限。遇到 `contract:<TASK_ID>` 时，运行 `uv run python -m omo.cli worker task contract-declare-deliverables <TASK_ID> --deliverables <PATH>... --actor <ACTOR> --now <ISO8601> --omo-dir .omo` 生成 request/proposal，然后走 approve/apply，再重新评估 overlay 直到它推进到 `launch:<TASK_ID>`。
...
| 一致性检查结果 | [CONSISTENCY-CHECK.md](../../_archive/CONSISTENCY-CHECK.md) |
| 战略蓝图 | [../design/MASTER-BLUEPRINT.md](../design/MASTER-BLUEPRINT.md) |
...
| 更新状态 | `../../state/system.yaml` | 通过 `omo state sync` 自动完成 |
...
1. `uv run python -m omo.cli state sync --omo-dir .omo`
2. `uv run python -m omo.cli worker task validate --all-active`
...
1. `uv run python -m omo.cli debt refresh --omo-dir .omo --now 2026-06-10T00:00:00Z`
2. `uv run python -m omo.cli debt dispatch --omo-dir .omo --now 2026-06-10T00:00:00Z`
3. `uv run python -m omo.cli debt campaign --omo-dir .omo`
4. `uv run python -m omo.cli debt report --omo-dir .omo`
5. `uv run python -m omo.cli state sync --omo-dir .omo`
...
- Revalidate command template: `uv run python -m omo.cli debt revalidate --omo-dir .omo --id <ID> --reviewed-at <RUN_AT>`
- Revalidate shell example: `uv run python -m omo.cli debt revalidate --omo-dir .omo --id <ID> --reviewed-at $(date -u +%Y-%m-%dT%H:%M:%SZ)`
...
- Approvals are recorded at `.omo/debt/approvals/<ITEM_ID>/current.yaml` and are created with `uv run python -m omo.cli debt approve --omo-dir .omo --id <ITEM_ID> --approved-by <OWNER> --scope execute_revalidate --approved-at <APPROVED_AT>`
...
- Generate the latest run-level coordination packet with `uv run python -m omo.cli debt campaign --omo-dir .omo`; pass `--run-ref <RUN_REF>` when you need to inspect a specific dispatch run instead of the latest one
...
- Generate the latest-run compact rollup with `uv run python -m omo.cli debt report --omo-dir .omo`; pass `--run-ref <RUN_REF>` when you need reporting for a selected run instead of the latest one
...
- Generate the cross-run history index with `uv run python -m omo.cli debt report-history --omo-dir .omo`
...
- Generate the latest-vs-prior diff with `uv run python -m omo.cli debt report-diff --omo-dir .omo`
...
- Generate the multi-run trend surface with `uv run python -m omo.cli debt report-trend --omo-dir .omo`
- Generate a bounded multi-run trend surface with `uv run python -m omo.cli debt report-trend --omo-dir .omo --last <N>`
- Generate an identity-bounded trend surface with `uv run python -m omo.cli debt report-trend --omo-dir .omo --from-run-stamp <STAMP> --to-run-stamp <STAMP>`
...
- Schedule command template: `uv run python -m omo.cli debt schedule --omo-dir .omo --id <ID> --next-review-at <NEXT_REVIEW_AT>`
- Schedule shell example: `uv run python -m omo.cli debt schedule --omo-dir .omo --id <ID> --next-review-at 2026-06-17T00:00:00Z`
...
| "我发现了不一致" | 更新对应 SSOT，运行 `_archive/CONSISTENCY-CHECK.md` 验证 |

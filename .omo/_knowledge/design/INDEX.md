...
> 本页是设计文档导航，不是阶段状态快照；计划状态以 `plans/README.md` 为目录级注册表，
> 运行时阶段与执行许可仍以 `/.omo/goals/current.yaml`、`/.omo/tasks/active/` 为准。
...
> 所有计划文档以 `plans/README.md` 为注册表入口。这里的状态仅是文档编目标签，
> 不是运行时 SSOT，也不代表当前仍可直接执行。

| 计划文档 | 状态 | 描述 |
|---------|------|------|
| [debt-cleanup-plan.md](debt-cleanup-plan.md) | REFERENCE | 债务清理方案（历史设计输入，当前债务以 `/.omo/debt/` 为准） |
...
| [phase5-entry-architecture.md](phase5-entry-architecture.md) | Phase 5 入口桥接设计：先完成 Phase 4 closeout，再进入 Entry Gate / Wave 0（历史入口设计输入） | reference |
| [phase5-program-architecture.md](phase5-program-architecture.md) | Phase 5 总纲设计：主 program、四平面边界、wave 结构与 go/no-go 规则（历史 program 设计） | reference |
...
| [plans/phase7-planning-analysis-requirements.md](plans/phase7-planning-analysis-requirements.md) | Phase 7 全面需求分析：10 章（架构/战略/红队/交叉分析/D1-D7，历史规划输入） | reference |
...
| [summaries/SB-DECISION.md](../summaries/SB-DECISION.md) | SharedBrain long-term decision：selective extraction + layered contraction（历史决策记录） | reference |
| [summaries/phase11-wave2-adr-ontoderive-inference-metatype.md](../summaries/phase11/phase11-wave2-adr-ontoderive-inference-metatype.md) | Wave 2 ADR：OntoDerive Inference adopts additive `meta_type`（历史 ADR） | reference |
| [summaries/phase11-wave2-adr-ontoderive-scheme-metatype.md](../summaries/phase11/phase11-wave2-adr-ontoderive-scheme-metatype.md) | Wave 2 ADR：OntoDerive Scheme adopts additive `meta_type`（历史 ADR） | reference |
| [summaries/phase11-wave2-adr-minerva-relation-metarelation.md](../summaries/phase11/phase11-wave2-adr-minerva-relation-metarelation.md) | Wave 2 ADR：Minerva Relation adopts additive `meta_relation`（历史 ADR） | reference |
| [summaries/phase11-wave4-adr-kos-canonical-metatype.md](../summaries/phase11/phase11-wave4-adr-kos-canonical-metatype.md) | Wave 4 ADR：KOS canonical MetaType SSOT aligns ingest and CLI（历史 ADR） | reference |
| [summaries/phase11-wave4-adr-eidos-protocol-contract-surface.md](../summaries/phase11/phase11-wave4-adr-eidos-protocol-contract-surface.md) | Wave 4 ADR：`eidos.protocols` extends from runtime Protocols to serialized payload contracts（历史 ADR） | reference |
...
| [system-design-baseline.md](system-design-baseline.md) | Post-Phase-14 canonical baseline：`4P3V1L` + `00/P0/I0/L1-L4/X1-X3` convergence and roadmap boundary | reference baseline |
...
| [hermes-convergence-strategy.md](hermes-convergence-strategy.md) | Hermes 收敛策略 — Phase 5 输入（历史输入，方向 A 主推荐 + 方向 B 参考） | reference |
...
| 文件 | 用途 | 状态 |
|------|------|------|
| [MASTER-BLUEPRINT.md](MASTER-BLUEPRINT.md) | 全景蓝图 v1.2（历史主蓝图，保留路线图与架构法则输入） | reference |
| [INSIGHTS-AND-ROADMAP.md](INSIGHTS-AND-ROADMAP.md) | 见解与路线图（历史洞察输入，不是当前路线图 SSOT） | reference |
...
*维护: 2026-06-17 · 计划文档状态以 plans/README.md 为准；运行时状态以 control/truth SSOT 为准*

...
> 本文档是当时的债务治理设计稿与收口记录，保留历史判断、方案与阶段门禁，不是当前债务账本或当前系统健康分 SSOT。
> 当前债务真相以 `/.omo/debt/registry.yaml`、`/.omo/debt/items/*.yaml` 为准；当前聚合状态只看 `/.omo/state/system.yaml` 的派生摘要。

---

## 零、历史状态记录 (2026-06-11 更新)

**以下内容记录当时的收口口径，不代表当前实时状态。** debt_weight / debt_health 等值仅作历史设计与验收上下文。

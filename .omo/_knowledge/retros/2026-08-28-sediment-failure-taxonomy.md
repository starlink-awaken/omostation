...
## 追加发现 (2026-08-28 晚间深挖)

**mini = 测试噪音**: run 文件实证 objective="real run test" actor=tester —
51 个 mini 失败全是测试 agent 的预期失败路径, 8/26 后测试停止归零。

**修正口径**: 153 = 测试噪音 51 (33%) + 治理拦截 41 (27%) + 真实生产失败 61
(40%, 全库约 12%) — sediment 23% 黑盒里真问题只有约 12%。

governance blocked 降级 info 收益已边际化 (08-28 全库仅 1 失败), 暂缓。

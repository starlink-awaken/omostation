...
| 2 | `governance_check_test_coverage()` | 每个非归档包至少 1 个 test_*.py | (无 GaC 规则 — 孤儿检查, Phase 2.3 已注册 check_type=test_coverage) | test_coverage |
...
| 4 | `governance_check_adr_links()` | ADR INDEX.md 引用的所有 ADR 都存在 | (无 GaC 规则 — ADR 链接完整性) | doc_lifecycle |
...
| 检查 | 状态 | 建议 |
|------|------|------|
| `governance_check_test_coverage()` | check_type=test_coverage 已注册 (Phase 2.3), 但无具体 GaC 规则 | 创建 `CR-X4-TEST-COVERAGE` 规则, target=kairon/packages/*/tests/ |
| `governance_check_adr_links()` | 无 GaC 规则 | 创建 `CR-X4-ADR-LINKS` 规则, target=.omo/_knowledge/decisions/ |

## 关系

- GaC `executor: [omo_audit]` 声明 → `run_governance_audit()` 是执行入口
- 本映射表是 **声明性文档**, 不替代代码中的实际检查逻辑
- `gac-drift.py` 验证 indexed 规则的 source_ref 文件存在 + ID 匹配, 但不验证 omo_audit 是否真正执行了该规则

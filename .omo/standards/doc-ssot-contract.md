...
`bin/doc-ssot-lint.py` 扫描所有 markdown 文件，检测:
...
python3 bin/doc-ssot-lint.py --fix        # 自动修复已知模式 (v5→v6 等)
python3 bin/doc-ssot-lint.py --json       # 机器可读 JSON (gac-healthcheck 消费)
```
...
| 执行器 (CI 门禁) | `bin/doc-ssot-lint.py` | 检测硬编码冲突, `--json` 供仪表盘 |
| 元治理自检 | `bin/gac-healthcheck.py` | doc_ssot 块 (ok/conflicts/files_scanned) |
...
3. `bin/doc-ssot-lint.py` — CI 门禁脚本 (检测硬编码冲突)

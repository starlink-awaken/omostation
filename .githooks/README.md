...
主仓 push 前自动把"本地领先远程"的子模块 push 上去,让 gitlink 可达,防 CI 悬空。子模块同步失败会阻断主仓 push。另执行 分支命名 / 直推 main 守卫 / reachability / gitlink 祖先 / 远程卫生 等检查（完整清单见 `hook-manifest.yaml`）。
...
合并后触发: 子模块指针变化通知 + 运行时产物/契约校验 + state-stale 检查 (非阻断 advisory)。
委托 `hook-runner.sh --hook post-merge`（完整清单见 `hook-manifest.yaml`）。
